...
class Groundwater(object):
    ...
    def __init__(self, iniItems,landmask,spinUp):
        ...
        # area/cells that can have groundwater abstraction
        aquifer_area = pcr.boolean(pcr.cover(vos.readPCRmapClone(\
             iniItems.groundwaterOptions['areaForGWAbstr'],
             self.cloneMap,self.tmpDir,self.inputDir), 0.0))
        self.non_aquifer = pcr.ifthenelse(aquifer_area, pcr.boolean(0.0), pcr.boolean(1.0))     
    ...
    def initialize_with_MODFLOW(self,iniItems,iniConditions):
        ...
        self.baseflowFromModflow = pcr.ifthen(self.landmask,\
                        vos.readPCRmapClone(iniItems.couplingToModflowOptions['baseflowFromModflow'],                                                   
                                            self.cloneMap,self.tmpDir,self.inputDir))                                  			  # unit: m3/day
        self.baseflow = self.baseflowFromModflow/self.cellArea   
        self.baseflow = pcr.cover((self.baseflow*-1.0),0.0)
                                                           						          # unit: m/day
        
        # use storGroundwater from the MODFLOW calculation/simulation:
        self.storGroundwaterFromModflow = pcr.ifthen(self.landmask,\
                               vos.readPCRmapClone(iniItems.couplingToModflowOptions['storGroundwaterFromModflow'],  
                                                   self.cloneMap,self.tmpDir,self.inputDir))                           		# unit: m3
        self.storGroundwater = self.storGroundwaterFromModflow/self.cellArea                                                # unit: m 
    ...
    def update_with_MODFLOW(self,landSurface,routing,currTimeStep):
        ...
        self.storGroundwater = self.storGroundwater      ## = stor groundwater from modflow
