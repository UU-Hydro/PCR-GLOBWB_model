...
for fileName in firstInitialConditionFileListMF: 
	cmd	= 'cp /home/inge/MODFLOW_coupled_TEST/iniCondHuman/*.map -d /projects/0/dfguu/users/inge/modflow_coupling_global_human/tmp/ini/'
	#cmd = 'cp /projects/0/dfguu/users/inge/inputMAPS/steadyState_MF/*.map -d /projects/0/dfguu/users/inge/modflow_coupling_global_human/tmp/ini/'
logger.info("Creating the master ini file for MODFLOW runs.")
vos.cmd_line(cmd, using_subprocess = False)
cmd	= 'cp /projects/0/dfguu/users/inge/modflow_coupling_global_human/tmp/ini/head_topMF.map -d /projects/0/dfguu/users/inge/modflow_coupling_global_human/'
vos.cmd_line(cmd, using_subprocess = False)

for fileName in firstInitialConditionFileListMF: 
	cmd	= 'cp /home/inge/MODFLOW_coupled_TEST/extrainputs/*.map -d /projects/0/dfguu/users/inge/modflow_coupling_global_human/tmp/ini/'
logger.info("Creating the master ini file for MODFLOW runs.")
vos.cmd_line(cmd, using_subprocess = False)
...
for i_date in range(1, modelTime.nrOfTimeSteps+1):
    ...
    # # - execute MODFLOW (globally)
      ...
      # execute MODFLOW
       ...
       #cmd = "python MODLFOW_ABSTR_1LAYER.py " +starting_date+" "+modelTime.fulldate
