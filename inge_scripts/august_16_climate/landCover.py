...
class LandCover(object):
    ...
    def calculateWaterDemand(self, parameters, \
        ...
        if isinstance(swAbstractionFraction, dict):
            ...
            # - as a function of the ratio between discharge and local baseflow (see the landSurface module) (after deGraaf2014)
        ...
        # no groundwater demand for non aquifer
        surface_water_demand = pcr.ifthenelse(groundwater.non_aquifer, self.totalGrossDemandAfterDesalination, surface_water_demand)
        ...
        # water demand that has been satisfied (unit: m/day)
        #
        ...
        # no groundwater demand for non aquifer
        self.potGroundwaterAbstract = pcr.ifthenelse(groundwater.non_aquifer, 0.0, self.potGroundwaterAbstract)
                
        # set the maximum abstraction to 5 cm per day
 ##########+======>>>>      # self.potGroundwaterAbstract = pcr.min(0.01, self.potGroundwaterAbstract)
        ...
        if groundwater.limitRegionalAnnualGroundwaterAbstraction:
            ...
            logger.debug('Total groundwater abstraction is limited by regional annual pumping capacity.')    ##-> deze geeft ie
            ...
            logger.debug('NO LIMIT for regional groundwater (annual) pumping. It may result too high groundwater abstraction.')   # -> maar deze geeft ie ook; komt denk ik omdat ie hier onder nog aan stond
        ...
        # no accesible water in non-aquifer = 
        readAvlStorGroundwater = pcr.ifthenelse(groundwater.non_aquifer, 0.0, readAvlStorGroundwater)
        #
