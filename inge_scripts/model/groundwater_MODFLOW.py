...
#Note: #* are changes made by Inge (to get right parameterization in there) 18/5/2016
...
class GroundwaterModflow(object):
    ...
    def __init__(self, iniItems, landmask):
        ...
        if 'estimateOfTotalGroundwaterThickness' in iniItems.groundwaterOptions.keys():
            ...
            #totalGroundwaterThickness = pcr.readmap("/projects/0/dfguu/users/Inge_MFcoupled/damc_ave")			#* werkt dit?
        ...
        if self.number_of_layers > 1 and self.iniItems.modflowParameterOptions['usePreDefinedConfiningLayer'] == "True":
            ...
            #self.confiningLayerThickness = cover(pcr.max(self.totalGroundwaterThickness*0.1,0.1),10)  #* 
    ...
    def set_grid_for_two_layer_model(self):
        ...
        bottom_layer_2       = cover(top_layer_2 - thickness_of_layer_2,-10) #* cover for ocena
        ...
        bottom_layer_1       = cover(bottom_layer_2 - thickness_of_layer_1,-100) #* added a cover for ocean (i think)
        ...
        if self.usePreDefinedConfiningLayer:
            ...
            # - make sure that the minimum thickness of layer 2 is at least 0.1 m
            thickness_of_layer_2 = pcr.max(0.1, top_layer_2 - bottom_layer_2)
            bottom_layer_2       = cover(top_layer_2 - thickness_of_layer_2,-10) #* cover for ocean
            # - thickness of layer 1 is at least 5.0 m
            thickness_of_layer_1 = pcr.max(5.0, total_thickness - thickness_of_layer_2)
            bottom_layer_1       = cover(bottom_layer_2 - thickness_of_layer_1,-100) #* added cover for ocean)
        
        #~ # report elevation in pcraster map
        pcr.report(pcr.ifthen(self.landmask, top_layer_2), "top_uppermost_layer.map")
        pcr.report(pcr.ifthen(self.landmask, bottom_layer_2), "bottom_uppermost_layer.map")
        pcr.report(pcr.ifthen(self.landmask, bottom_layer_1), "bottom_lowermost_layer.map")
    ...
    def set_bcf_for_two_layer_model(self):
        ...
        horizontal_conductivity = pcr.cover(pcr.ifthenelse(self.estimateConfinedLayers > 0.0 , 0.01,self.kSatAquifer),self.kSatAquifer) #*
        ...
        if self.usePreDefinedConfiningLayer:
            ...
            vertical_conductivity_layer_2 = pcr.cover(pcr.ifthenelse(self.estimateConfinedLayers > 0.0, vertical_conductivity_layer_2 * 0.1, vertical_conductivity_layer_2),vertical_conductivity_layer_2) #*
        ...
        #~ # report khoriz_l1 kvert_l1 in pcraster map
        pcr.report(pcr.ifthen(self.landmask, horizontal_conductivity_layer_1), "horizontal_conductivity_layer_1.map")
        pcr.report(pcr.ifthen(self.landmask, horizontal_conductivity_layer_2), "horizontal_conductivity_layer_2.map")
        pcr.report(pcr.ifthen(self.landmask, vertical_conductivity_layer_1), "vertical_conductivity_layer_1.map")
        pcr.report(pcr.ifthen(self.landmask, vertical_conductivity_layer_2), "vertical_conductivity_layer_2.map")
        
        # set conductivity values to MODFLOW
        horizontal_conductivity_layer_2= cover(horizontal_conductivity_layer_2,10.0)   #* 
        vertical_conductivity_layer_2= cover(vertical_conductivity_layer_2,10.0) #*
        horizontal_conductivity_layer_1= cover(horizontal_conductivity_layer_1,10.0) #*
        vertical_conductivity_layer_1= cover(vertical_conductivity_layer_1,10.0) #*
