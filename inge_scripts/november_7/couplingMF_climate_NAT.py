...
print(firstInitialConditionFilesMF)
...
for fileName in firstInitialConditionFileListMF: 
	#cmd	= 'cp /home/inge/MODFLOW_coupled_TEST/iniCondNatural/*.map -d /projects/0/dfguu/users/inge/test_modflow_coupling_global_natural/tmp/ini/'
	cmd= 'cp /projects/0/dfguu/users/inge/inputMAPS/steadyState_MF/*.map -d /projects/0/dfguu/users/inge/modflow_coupling_global_natural/tmp/ini/'
logger.info("Creating the master ini file for MODFLOW runs.")
vos.cmd_line(cmd, using_subprocess = False)
cmd	= 'cp /projects/0/dfguu/users/inge/modflow_coupling_global_natural/tmp/ini/head_topMF.map -d /projects/0/dfguu/users/inge/modflow_coupling_global_natural/'
vos.cmd_line(cmd, using_subprocess = False)
...
for i_date in range(1, modelTime.nrOfTimeSteps+1):
    ...
    # # - execute MODFLOW (globally)
      ...
      # execute MODFLOW
       #cmd = 'mkdir '+pcrglobwbMonthlyOutputFolder+'/global'; vos.cmd_line(cmd, using_subprocess = without_debug)
       #cmd = 'mkdir '+pcrglobwbMonthlyOutputFolder+'/global/netcdf'; vos.cmd_line(cmd, using_subprocess = without_debug)
      
       cmd = "python MF_climate_NAT.py " +starting_date+" "+modelTime.fulldate
       print(cmd)
       os.system(cmd)
     ...
     # PCR-GLOBWB copying the initial condition to a temporary ini directory
       #cmd = "cp "+generalOutputFolder+"/merged/states/*"+str(modelTime.fulldate)+"*.map "+generalOutputFolder+"/tmp/ini/"
       #print(cmd)
     ...
     #cmd = "for f in "+generalOutputFolder+"/tmp/ini/*"+date_for_initial_maps+"*; do mv -v ""$f"" $(echo ""$f"" | tr '"+date_for_initial_maps+"' 'xxxx-xx-xx'); done"
     #print(cmd)
     #os.system(cmd)
