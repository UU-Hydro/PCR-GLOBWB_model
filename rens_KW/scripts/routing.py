...
# NOTE: changes have been made to the way the script handles the inundated fraction
# when the floodplain option is employed
# TODO: check computation of the wetted perimeter
...
class Routing(object):
    ...
    def __init__(self,iniItems,initialConditions,lddMap):
        ...
        if self.floodPlain:
            ...
            self.channelFraction= ifthen(self.landmask,pcr.cover(self.constantChannelWidth,0))*self.channelLength/self.cellArea
            ...
            pcr.report(self.cellArea,'cellArea.map')
            pcr.report(self.channelFraction,'channelFraction.map')
            pcr.report(self.constantChannelWidth,'channelWidht.map')
    ...
    def update(self,landSurface,groundwater,currTimeStep,meteo):
        ...
        #-added optional computation of channel fraction when not yet initialized using the floodplain option
        ...
        if self.floodPlain:
            ...
            self.dynamicFracWat = pcr.max(self.channelFraction, self.WaterBodies.dynamicFracWat) #-self to channelfraction added
        ...
        self.test1 = self.WaterBodies.dynamicArea / self.cellArea                        
        self.test2 = self.WaterBodies.dynamicArea * self.WaterBodies.waterBodyRelativeFrac
        self.test3 = self.WaterBodies.dynamicFracWat
    ...
    def returnFloodedFraction(self,channelStorage):
        ...
        floodedFraction= pcr.max(0.,pcr.min(1.,floodedFraction+self.channelFraction))
    ...
    def kinAlphaDynamic(self,channelStorage):
        ...
        wetPCh= self.wMean+\
            2.*(pcr.min(self.channelDepth,channelStorage/(self.channelLength*self.wMean))+floodZ)
        wetP= wetPFld+wetPCh
