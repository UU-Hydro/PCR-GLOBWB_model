...
# - start the run
SPINUP=$(sbatch --nodelist "${NODENMR}" -J "${SUB_JOBNAME}" -n 72 --export INI_FILE="${SUB_INIFILE}",MAIN_OUTPUT_DIR="${SUB_OUT_DIR}",STARTING_DATE="${STA_DATE}",END_DATE="${END_DATE}",MAIN_INITIAL_STATE_FOLDER="${INITIAL_FOLD}",DATE_FOR_INITIAL_STATES="${INITIAL_DATE}",BASEFLOW_EXPONENT="${BFEXPON}" job_script_sbatch_pcrglobwb_template.sh | sed 's/Submitted batch job //')
...
# - start the run
FIRST=$(sbatch --nodelist "${NODENMR}" --dependency=afterany:${SPINUP} -J "${SUB_JOBNAME}" -n 72 --export INI_FILE="${SUB_INIFILE}",MAIN_OUTPUT_DIR="${SUB_OUT_DIR}",STARTING_DATE="${STA_DATE}",END_DATE="${END_DATE}",MAIN_INITIAL_STATE_FOLDER="${INITIAL_FOLD}",DATE_FOR_INITIAL_STATES="${INITIAL_DATE}",BASEFLOW_EXPONENT="${BFEXPON}" job_script_sbatch_pcrglobwb_template.sh | sed 's/Submitted batch job //')
