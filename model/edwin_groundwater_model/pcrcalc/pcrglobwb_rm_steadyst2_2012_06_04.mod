...
#
  gw_head_INI	 =   results\gw_headd.ini			;		# +m 		result of steady state groundwater model without ADDQ3 (fast response baseflow)
#
...
# TIME SERIES OUTPUT MAPS and TSSs:
  gw_head	 = results\gw_headd.map				;		#  m 		groundwater head - daily   # from MODFLOW
  gw_baseflow	 = results\bsfl					;		#  m3/day 	groundwater baseflow from RIV + DRN package (positive entering the aquifer)
  gw_base_mpd	 = results\bsfl_mpd				;		#  m/day 	groundwater baseflow from RIV + DRN package (positive entering the aquifer)											
  gw_head_con    = results\gw_hdcon.map				;		#  m 		to check if the steady state condition has achieved
  CRFRAC_RIV     = results\crfracrv.map 			; 		#  -		
  rivhead        = results\rivheadd.map    			; 		# (m)		river waterlevel - daily 					# ONLY END REPORT
...
###################################################################################################################################################
  ...
  res = mf::setPCG(999,1250, 1, 0.100,100, 0.98, 2, 1)  ;	# res    = mf::setPCG(MXITER, ITERI, NPCOND, HCLOSE, RCLOSE, RELAX, NBPOL, DAMP) ;
...
#
  ...
  waterlv = if(waterlv lt 0.01, scalar(0), waterlv)							;	# if there are only 1 cm water in the channel, there are no exchange between surface-water and ground-water
...
report (rep_y) CRFRAC_RIV = if(LANDMASK, CRFRAC_RIV)							;
#
...
report (rep_y) rivhead     = if(LANDMASK,rivhead_ALL) 			;
               rivhead     =  cover(rivhead_ALL, scalar(-88888))	;
#
# CONSTANT RIVER PACKAGE for simulating THE RIVER NETWORK  
#########################################################################################################################################################
# adding the RIV package		;
  rivbott_combined = if(rivcond gt 0, rivbott, rivhead)			;
  rivcond_combined = if(rivcond gt 0, rivcond, drncond)			; 
  res = mf::setRiver(rivhead,rivbott_combined,rivcond_combined,1)	;

# DRAIN PACKAGE for simulating the linear reservoir of S3  
#################################################################################################################################################################
  base_S3used = cover(BASE_S3, scalar(-9000))				;
  KQ3_x_Sy    = min(1, cover(KQ3 * scoef_act, scalar(0)))		;
  KQ3_x_Sy_AR = if(base_S3used eq -9000, scalar(0), KQ3_x_Sy)*CELLAREA	;
  res = mf::setDrain( base_S3used, KQ3_x_Sy_AR, 1)			; # DRAIN PACKAGE for simulating the linear reservoir.
...
# INITIAL CONDITION of groundwater head
#########################################################################################################################################################
  gw_head  	= if(LANDMASK, gw_head_INI_FULL)	;
...
# RCH = cover((R3-ADDQ3)*CELLAREA/(3000*3000),scalar(0))		;	# not USED
  RCH = cover((R3      )*CELLAREA/(3000*3000),scalar(0))		;	#     USED
...
#
  ...
  report (rep_y) gw_head     =           if(LANDMASK, gw_head_mod)			;
  report (rep_y) gw_head_con = if(LANDMASK,mapmaximum(abs(gw_head - gw_head_INI_FULL))) ;
