...
def file_is_nc(filename):
    '''
file_is_nc: tests if the file extension matches that of a netCDF file;
...
def file_is_pcr(filename):
    '''
file_is_pcr: tests if the file extension matches that of a PCRaster file;
returns True if this is the case.
'''
...
def file_is_raster(filename):
    '''
file_is_raster: tests if the file extension matches that of a spatial dataset;
returns True if this is the case.
'''
    ...
    # return the test condition
    if not file_ext in file_extensions.keys():

        spatialAttributes(filename)
...
given date. This may concern spatial information or single entries.
    ...
    allow_year_substitution:
                            ...
                            (default: False);
    conv_constant,
    conv_factor:            conversion constant and factor applied to var_out
                            if this is a scalar variable or float; default
                            values are zero and one respectively (no change).
...
'''
    ...
    elif existing_file and (file_is_pcr(filename) or file_is_raster(filename)):
    ...
    if datatype_str.title() == 'Scalar':
        if not isinstance(var_out, NoneType):
            var_out = conv_constant + conv_factor * var_out
...
class spatio_temporal_fields(object):

    """

spatio_temporal_fields: class that holds the spatio-temporal fields with the \
dates as key and the corresponding spatio-temporal field as entry; this class \
is intended to keep information in memory when the temporal density of \
information is sparse and reading the information requires relatively much \
processing time whereas the amount of information is relatively light. For \
example, it is intended to be used with monthly data.
...
"""
    ...
    # initialization #
    ##################
    def __init__(self, spatio_temporal_field_info, \
                       variablenames    = {}, \
                       suffixes         = [], \
                       clone_attributes = None):
...
__init__: initialization of an instance of the spatio_temporal_fields_class.

    Input:
    ======
    spatio_temporal_field_info  : a dictionary linking the variable name to a
                                  file with the spatio-temporal information,
                                  e.g., a netCDF file;
    clone_attributes             : instance of the spatialAttributes class that
                                  holds information on the spatial extent and
                                  resolution of the maps to be generated;
                                  default value is None, in which case it has
                                  to be passed when maps are extracted.
...
'''
        ...
        # set the information from the dictionary on the spatio-temporal fields
        self.spatio_temporal_field_info = {}
        self.variablenames = variablenames.copy()

        # iterate over the files and find the suffixes
        for key in spatio_temporal_field_info.keys():

             if key not in self.variablenames.keys():

                # initialize the variable name
                variablename = key[:]

                # try to remove the suffixes
                for suffix in suffixes:

                    if suffix in key:

                        start_pos = key.find(suffix)

                        if start_pos + len(suffix) == len(key):

                            variablename = key.replace(suffix, '')


                # set the information
                self.spatio_temporal_field_info[variablename] = \
                                          spatio_temporal_field_info[key]
                self.variablenames[variablename] = variablename

             else:

                self.spatio_temporal_field_info[key] = \
                                          spatio_temporal_field_info[key]

        # set the clone information
        self.clone_attributes = clone_attributes

        # initialize all variables
        for variablename in self.variablenames:

            setattr(self, variablename, dict())

        # returns None
        return None


    def read_fields_for_dates(self, dates, \
                                    variablenames = [], \
                                    reset_values  = False, \
                                    file_info      = {}):
...
read_field_for_date: function of the class spatio_temporal_fields that returns \
the information for a selected date for the file information provided using the \
read_file_entry function of the file handler module.
    ...
    Output:
    =======
    None                : returns None; resets internally the
...
'''
        ...
        if len(variablenames) == 0:
                ...
                variablenames = list(self.spatio_temporal_field_info.keys())
        ...
        for variablename in variablenames:
            ...
            for key, value in file_info.items():
                ...
                var_file_info[key] = value

            # add the file name and the variable file
            var_file_info['filename']     = self.spatio_temporal_field_info[variablename]
            var_file_info['variablename'] = self.variablenames[variablename]

            if not isinstance(self.clone_attributes, NoneType) and \
                              'clone_attributes' not in var_file_info.keys():

                var_file_info['clone_attributes'] = self.clone_attributes
            ...
            # get the variable
            var_obj_info = getattr(self, variablename)

            # iterate over the dates
            for date in dates:

                if date not in var_obj_info.keys():

                    var_obj_info[date] = self.read_field_for_date( \
                                              date, var_file_info)

            # set the information on the variable
            setattr(self, variablename, var_obj_info)

        # returns None
        return None


    def read_field_for_date(self, date, file_info):
...
'''
        ...
        return read_file_entry( \
                    ...
                    variablename            = file_info['variablename'] , \
                    inputpath               = file_info['inputpath'] , \
                    file_subst_args         = file_info['file_subst_args'],\
                    clone_attributes        = file_info['clone_attributes'] , \
                    forced_non_spatial      = file_info['forced_non_spatial'], \
                    datatype                = file_info['datatype'] , \
                    date                    = date, \
                    date_selection_method   = file_info['date_selection_method'] , \
                    allow_year_substitution = file_info['allow_year_substitution'] )
