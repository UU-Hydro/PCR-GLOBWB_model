...
from xarray_functions import get_fillvalue, open_xr_dataset, close_xr_dataset, \
                             get_xr_variable_array
...
'''
    ...
    elif isinstance(dates, np.ma.core.MaskedArray):
        new_dates = np.array(dates.tolist())
        new_dates = new_dates.ravel()
...
'''
    ...
    if np.size(dates[time_delta == datetime.timedelta(0)]) > 0 and \
        ...
        return date_index[time_delta == datetime.timedelta(0)][0]
        ...
        return date_index[time_delta == time_delta.min()][0]
...
# TODO: incorporate the year substitution in match_date_in_dates
...
'''
    ...
    while variable_array.size != req_size:
        print (variable_array.size, req_size)
        sys.exit('array sizes of %s do not match!' % name)
...
holding netCDF information to facilitate access.'''
        ...
        if not ncfilename in self.cache.keys():
            ...
            self.cache[ncfilename] = open_xr_dataset(ncfilename)

            # get the time variable
            time_dimension = None
            for variablename in self.dimensions[ncfilename].keys():

                if variablename in self.variables[ncfilename].keys() and \
                            'calendar' in self.variables[ncfilename][variablename]:
                    time_dimension = variablename
            ...
            for variablename in self.variables[ncfilename].keys():
                ...
                # test on the size and set processing of variables that
                # are not dimensions
                if len(nc_dims) > 0:
                ...
                if process_non_dimensional_variable:
                    ...
                    if not forced_non_spatial and len(nc_dims) >= 2:
                        ...
                        if not isinstance(data_attributes, NoneType):
                            ...
                            # print(same_clone, same_resolution)
                            # for key in vars(clone_attributes).keys():

                            #     print(key, vars(clone_attributes)[key], \
                            #           vars(data_attributes)[key])
...
specified netCDF file.'''
        ...
        if ncfilename in self.cache.keys():
            ...
            close_xr_dataset(self.cache[ncfilename])
...
'''
        ...
        if self.variables[ncfilename][variablename]['timed_variable']:
            ...
            # year substitution is allowed, find the corresponding match
                ...
                if isinstance(date_index, NoneType):
                    ...
                    # replace the date in the years
                    substituted_dates = substitute_years_of_dates(dates, replacement_date)
                    
                    # and get the corresponding date index
                    date_index = get_date_index( \
                            date, substituted_dates, date_selection_method, \
                            substituted_dates = True)
        ...
        if self.variables[ncfilename][variablename]['spatial_variable']:
            ...
            if not self.spatialattributes[ncfilename][variablename]:
                ...
                if datatype == pcr.Ldd or datatype == str(pcr.Ldd) and not same_resolution:

                    # raise error if not the same resolution

                    logger.error('data of type LDD as read from %s for %s cannot be resampled' % \
                                 (ncfilename, variablename))

                # if the same resolution, use xarray
                if same_resolution:
                    ...
                    for match_key in ['lat', 'latitude']:
                        if match_key in self.dimensions[ncfilename].keys():
                            lat_key = match_key

                    for match_key in ['lon', 'longitude']:
                        if match_key in self.dimensions[ncfilename].keys():
                            lon_key = match_key

                    if not isinstance(lon_key, NoneType):
                        sample_info[lon_key] = {'values' : [clone_attributes.xLL, \
                                                            clone_attributes.xUR], \
                                                'slice'  : True}

                    if not isinstance(lat_key, NoneType):
                        sample_info[lat_key] = {'values' : [clone_attributes.yUR, \
                                                            clone_attributes.yLL], \
                                                'slice'  : True}

                    if self.variables[ncfilename][variablename]['timed_variable']:
                        sample_info[self.time_dimension[ncfilename]] = {'values' : [matched_date]}


                    # read in the information from the netCDF file
                    var_array = get_xr_variable_array(self.cache[ncfilename], \
                                              variablename, \
                                              **sample_info)

                    var_array = var_array.values
                    var_array[np.isnan(var_array)] = self.variables[ncfilename][variablename][nc_mv_id_str]
                    var_array.shape = (clone_attributes.numberRows, clone_attributes.numberCols)
            ...
            var_out = pcr.ifthen(var_out != self.variables[ncfilename][variablename][nc_mv_id_str],\
                                     var_out)
...
'''
    ...
    def get_general_netcdf_attributes(self, model_configuration):
        ...
        # netCDF attributes
        scenarioname = ''
        for key in ['scenario', 'scenarioname']:
            if key in model_configuration.general.keys():
                scenarioname = model_configuration.general[key]
...
'''
        ...
        if 'is_temporal' in additional_info.keys():
            is_temporal = additional_info['is_temporal']
