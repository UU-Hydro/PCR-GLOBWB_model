...
def getNCDateIndex(date, dates, select= 'exact'):
	'''Returns the position of the date specified in an array of dates,\
 following the index selection method specified: exact, before, after, nearest.\
 ...
 circumvents the problem that the original function cannot read from a standard dictionary.'''
	#-create array with timeDelta objects
	timeDelta= dates.copy()
	timeDelta-= date
	#-find zero value
	if np.size(dates[timeDelta == datetime.timedelta(0)]) > 0:
		return np.arange(dates.size)[timeDelta == datetime.timedelta(0)][0]
	elif select == 'exact':
		return None
	elif select == 'before':
		maskValue= timeDelta.min()
		timeDelta[timeDelta >=  datetime.timedelta(0)]= maskValue
		return np.arange(dates.size)[timeDelta == timeDelta.max()][0]
	elif select == 'after':
		maskValue= timeDelta.max()
		timeDelta[timeDelta <=  datetime.timedelta(0)]= maskValue
		return np.arange(dates.size)[timeDelta == timeDelta.min()][0]
	elif select == 'nearest':
		timeDelta= np.abs(timeDelta)
		return np.arange(dates.size)[timeDelta == timeDelta.min()][0]
	else:
		sys.exit('index selection method %s is not allowed or does not yield a result' % select)
...
def readField(ncFile, variableName, posCnt):
	#~ print ncFile
	rootgrp= nc.Dataset(ncFile,'r')
	#- read data
...
def main():
	...
	testNCFile= '/home/rens/netData/GlobalDataSets/CRU_TS3.21/data/tmp/cru_ts3.21.tmp.dat.nc'
