...
# cru input
input_cru = {}
input_cru['folder'] = "/data/hydroworld/basedata/forcing/CRU_TS3.23"
...
input_ecmwf = {}
input_ecmwf['files'] = {}
input_ecmwf['files']['rainfall']           = os.path.join('/data/hydroworld/basedata/forcing/ERA20C','ERA20C_daily_totp_1901_to_2010.nc')
input_ecmwf['files']['temperature']        = os.path.join('/data/hydroworld/basedata/forcing/ERA20C','ERA20C_daily_tavg_1901_to_2010.nc')
...
ncVariableNames= {'temperature': 'var167', 'rainfall': 'var228', 'evaporation': 'var167'}
...
def main():
    ...
    cruFileRoot= 'cru_ts3.23.1901.2014.%s.%s.nc'
    ...
    #~ # print content of all variables
    #~ for variable in meteoVariables.keys():
      #~ print variable
      #~ for key, value in meteoVariables[variable].iteritems():
        #~ print key, value
...
################### RvB: added ###########################################
                    ...
                    for dayCnt in xrange(numberDays):
                        day= dayCnt+1
                        fileName = (meteoVariables[variable]['sourceFileName'])
                        ...
                        #~ print  fileName, ncVariableNames[variable]
...
##########################################################################
                ...
                #~ pcr.report(pcr.numpy2pcr(pcr.Scalar,weight[variable],vos.MV),'weight_%s.%03d' % (variable,month))
                ...
                if variable == 'evaporation':
                    fileName = meteoVariables[variable]['ecmwf_total']
                    ncFileFormat,ncAttributes, ncDimensions, ncVariables= rvb_ncR.getNCAttributes(fileName)
                    ncDates= rvb_ncR.getNCDates(ncDimensions['time'],ncVariables['time'])
                    ncDate= datetime.datetime(year,month,1)
                    posCnt= rvb_ncR.getNCDateIndex(ncDate,ncDates)                    
                    targetMonthlyValue[variable] = rvb_ncR.readField(fileName,'potentialEvaporation',posCnt)
