...
# output folder
outputFolder = "/projects/wtrcycle/users/edwinhs/cru_erainterim_forcing/"
try:
...
front_filename = "cruts321_erainterim"

# clone map file name (global with 0.50 degree cell size)
cloneMapFileName = "/projects/dfguu/data/hydroworld/others/Global/Global_CloneMap_30min.map"

# start year, end year: full ERA-Interim years (default)
startYear = 1979 # 2000 # 1979
endYear   = 2010 # 2000 # 2010
#
...
# cru input
input_cru = {}
input_cru['folder'] = "/projects/dfguu/data/hydroworld/basedata/forcing/CRU_TS3.21"
...
input_snow_correction['folder'] = "/projects/dfguu/data/hydroworld/basedata/forcing/snowCorrection"
...
def main():
    ...
    logger.info('Correcting daily ERA Interim fields on the basis of CRU monthly values')
    ...
    netcdf_attribute_dictionary['comment'    ]  = "Prepared and calculated by Edwin Sutanudjaja. The script is inspired by Rens van Beek. "
    netcdf_attribute_dictionary['comment'    ] += "The main datasets used are CRU TS3.21 and ERA Interim datasets. "
    netcdf_attribute_dictionary['comment'    ] += "GPCP datasets are also used particularly for areas supported by no or very limited CRU stations. "
    ...
    for year in xrange(startYear, endYear+1):
        ...
        for month in xrange(1,12+1):
            ...
            for variable in meteoVariables.keys():        
                ...
                logger.info('The covering dataset used is from '+str(cover_dataset))
                ...
                for cruVarName in meteoVariables[variable]['stn']:
                    ...
                    # add the weicght for the current variable to the overall weight
