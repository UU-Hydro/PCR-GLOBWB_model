...
class DeterministicRunner(pcrfw.DynamicModel):
    ...
    def __init__(self, configuration, modelTime, initialState, ldd_lue,\
                 count: int,
                 nr_workers: int,
...
def main(
    ...
    # - set the initial_state to None, so it will be based on the configuration .ini file
    initial_state = None
    ...
    # for LUE experiment, read/set all input here (note only ldd is needed for the LUE experiment)
    ldd_lue = None
    if pcr.provider_name == "lue":
    ...
    deterministic_runner = DeterministicRunner(configuration, currTimeStep, initial_state, ldd_lue, count, nr_workers, array_shape, partition_shape, result_pathname, centre)
