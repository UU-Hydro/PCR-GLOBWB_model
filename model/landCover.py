...
class LandCover(object):
    ...
    def __init__(self,iniItems,NameOfSectionInIniFile,parameters,landmask,usingAllocSegments = False):
        ...
        self.fracVegCover = pcr.ifthenelse(self.fracVegCover > 0.0005, self.fracVegCover, 0)
    ...
    def calculateWaterDemand(self, parameters, nonIrrGrossDemand, swAbstractionFraction, groundwater, routing, allocSegments):
        ...
        totalGrossDemand = self.nonIrrGrossDemand + self.irrGrossDemand
        ...
        if self.usingAllocSegments:      # using zone/segment at which supply network is defined
            ...
            ADJUST = pcr.ifthenelse(ADJUST>0.0, \
                     pcr.min(1.0,pcr.max(0.0,segAvlSurfaceWater) / ADJUST),0.)
            #
            ADJUST = pcr.rounddown(ADJUST*100.)/100.
            ADJUST = pcr.max(0.0, ADJUST)
            ADJUST = pcr.min(1.0, ADJUST)
            #
            ...
            volActSurfaceWaterAbstract = pcr.ifthenelse(segAvlSurfaceWater > 0.0, \
                                              routing.readAvlChannelStorage/\
                                              segAvlSurfaceWater, 0.0)*\
                                             (segActSurWaterAbs)                                                 
            volActSurfaceWaterAbstract = pcr.min(routing.readAvlChannelStorage, volActSurfaceWaterAbstract)  # unit: m3
            # avoid small values (to avoid rounding error)
            volActSurfaceWaterAbstract = pcr.ifthenelse(volActSurfaceWaterAbstract > 1.0, volActSurfaceWaterAbstract, 0)
            ...
            segActSurWaterAbs = pcr.areatotal(self.actSurfaceWaterAbstract *routing.cellArea, allocSegments)           # unit: m3
            
            # allocation surface water abstraction volume to each cell (unit: m3)
            self.volAllocSurfaceWaterAbstract = pcr.ifthenelse(segTtlGrossDemand > 0.0,
                                cellVolGrossDemand/segTtlGrossDemand, 0.0)*\
                                segActSurWaterAbs                                                    # unit: m3 
            self.volAllocSurfaceWaterAbstract = pcr.cover(self.volAllocSurfaceWaterAbstract, 0.0)    # TODO: Do we really have to cover?
            ...
            if self.debugWaterBalance == str('True'):
                ...
                abstraction = pcr.cover(pcr.areatotal(pcr.cover(self.actSurfaceWaterAbstract  *routing.cellArea, 0.0), self.allocSegments)/self.segmentArea, 0.0)
                allocation  = pcr.cover(pcr.areatotal(pcr.cover(self.allocSurfaceWaterAbstract*routing.cellArea, 0.0), self.allocSegments)/self.segmentArea, 0.0)
            ...
            self.actSurfaceWaterAbstract = pcr.cover(self.actSurfaceWaterAbstract, 0.0)
