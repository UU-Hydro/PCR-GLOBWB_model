...
class PCRGlobWB(object):
    ...
    def checkWaterBalance(self, storesAtBeginning, storesAtEnd):
        ...
        if self.landSurface.usingAllocSegments:
            ...
            allocNonFossilGroundwaterAbs = nonFossilGroundwaterAbs   # PS: We assume that nonFossilGroundwaterAbs is extracted (only) to satisfy local demand.
            ...
            vos.waterBalanceCheck([segTotalDemand],\
                                  ...
                                  [pcr.scalar(0.0)],\
                                  [pcr.scalar(0.0)],\
                                  'water demand partitioning (per zone)',\
                                   True,\
                                   self._modelTime.fulldate,threshold=1e-3)
        else:    
