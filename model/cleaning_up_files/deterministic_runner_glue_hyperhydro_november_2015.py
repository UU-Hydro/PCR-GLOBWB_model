...
class DeterministicRunner(DynamicModel):
    ...
    def adusting_parameters(self, configuration, system_argument): 
        ...
        multiplier_for_degreeDayFactor  = float(system_argument[4])
        multiplier_for_kSat             = float(system_argument[5])
        multiplier_for_recessionCoeff   = float(system_argument[6])
        multiplier_for_manningsN        = float(system_argument[7])    
        ...
        msg += "For manningsN                  : "+str(multiplier_for_manningsN       )+"\n"
        logger.info(msg)
        # - also to a txt file 
        ...
        self.model.routing.manningsN = pcr.max(0.0, multiplier_for_manningsN * self.model.routing.manningsN)
        # maximum value is floodplainManN
        self.model.routing.manningsN = pcr.min(self.model.routing.manningsN, self.model.routing.floodplainManN)
        # report the map
        pcr.report(self.model.routing.manningsN, "manningsN")

        # set parameters "kSat" and "degreeDayFactor" based on the given pre-multipliers
