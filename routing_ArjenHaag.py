...
# NOTES:
# - Added a second version of accuTravelTime, which works exactly the same way
#   as the original version, except that it uses 'channelWidth' instead of 'wMean'
#   and 'channelLength' instead of the cell diagonal distance.
#	There are read from data, and this served to analyse its effects.
# - How is lateral inflow (or outflow) handled with kinematic wave now?!
#   -> currently, lateral inflow is handled in def update, where 'channelStorage'
#       is updated, routing is performed, and then storage is updated again.
#		this works similar as "accuTravelTIme"
#   -> in Rens' DynRout lateral inflow is handled within the loop of kinematic wave,
#       by 'deltaActualStorage' (diving all in/out flows by 'nrIter').
#       see lines 302-332. This should still be fixed here!
...
class Routing(object):
    ...
    def __init__(self,iniItems,initialConditions,lddMap):
        ...
        # CHANGED: reading routing option from ini file
        #---------------------------------------------------------------------
        ...
        # CHANGED: parameters originally within 'if-loop' for accuTravelTime,
        # but still required in certain other areas of script (def update)
        #---------------------------------------------------------------------
        self.eta = 0.25
        self.nu  = 0.40
        self.tau = 8.00
        ...
        distanceDownstream = pcr.ldddist(self.lddMap,self.lddMap == 5,\
                                                         self.cellLengthFD)
        channelLengthDownstream = (self.cellLengthFD + distanceDownstream)/\
                                    (nrCellsDownstream + 1)                     # unit: m
        self.dist2celllength  = channelLengthDownstream / self.cellSizeInArcDeg # unit: m/arcDegree  
        #---------------------------------------------------------------------

        # original accuTravelTime
        ...
        # ADDED: new accuTravelTime (using 'channelWidth' and 'channelLength')
        # and kinematic wave parameters
        #---------------------------------------------------------------------
        elif self.method == "accuTravelTime2":
            
            routingParameters = ['gradient','manningsN','channelLength','channelWidth']
            for var in routingParameters:
                input = iniItems.routingOptions[str(var)]
                vars(self)[var] = vos.readPCRmapClone(input,\
                                self.cloneMap,self.tmpDir,self.inputDir)
        
        elif self.method == "KinematicWave":
            ...
            # function 'kinematic' requires time input in seconds
            self.timeSec = float(iniItems.globalOptions['timeStep']) * vos.secondsPerDay()
            
            # default number of iterations, used to calculate sub time steps
            # (value taken from DynRout script of Rens)
            self.nrIterDefault = 12.
            
            # beta (constant of kinematic wave equation) [-]
            self.betaQ = 0.6
        #---------------------------------------------------------------------
    ...
    def getICs(self,iniItems,iniConditions = None):
        ...
        # ADDED: initial conditions for kinematic wave
        #---------------------------------------------------------------------
        if self.method == "KinematicWave":
            
            # initial Q
            # set equal to channel storage for now (which is 0), CHANGE LATER?!
            self.Q = self.channelStorage
            
            # initial values for wetted area, alpha and flow velocity
            self.wettedArea,self.alphaQ = self.kinematicAlpha(self.channelStorage)
            # Why this second update of wettedArea? see also .txt file notes
    ...
    def update(self,landSurface,groundwater,currTimeStep,meteo):
        ...
        # CHANGED: if-loop for routing parameters, waterBodies parameters and
        # simulation of water bodies fraction to distinguish between the use
        # of 'wMean' for accuTravelTime and 'channelWidth' for other routing
        #---------------------------------------------------------------------
        if (self.method == "accuTravelTime") or (self.method == "accuTravelTime2"):
        ...
        # CHANGED: check for routing options and run selected routing scheme
        #---------------------------------------------------------------------
        if self.method == "KinematicWave":
            self.KinematicWave(currTimeStep)
        else:
        ...
        # total water storage volume (m3) for the entire column 
        # (including interception, snow, soil and groundwater) 
    ...
    def getRoutingParamAvgDischarge(self, avgDischarge, dist2celllength):
        ...
        # CHANGED: if-loop to distinguish between 'wMean' and 'channelWidth'.
        # wMean used for original routing, channelWidth for new versions of 
        # accuTravelTime and KinematicWave routing.
        #---------------------------------------------------------------------
        if self.method == "accuTravelTime":        
            ...
            characteristicDistance = \
                 ( (yMean *   self.channelWidth)/ \
                   (self.channelWidth + 2*yMean) )**(2./3.) * \
                  ((self.gradient)**(0.5))/ \
                    self.manningsN * \
                    vos.secondsPerDay()                       #  meter/day
        #---------------------------------------------------------------------
    ...
    def kinematicAlpha(self,watStor):
        # Uses the total water storage in a cell to calculate the 
        # wetted area and alpha parameter for the kinematic wave
        wetA = watStor/self.channelLength
        wetP = 2.*wetA/self.channelWidth+self.channelWidth
        alphaQ = (self.manningsN*wetP**(2./3.)*self.gradient**-0.5)**self.betaQ
        return wetA,alphaQ
    ...
    def KinematicWave(self,currTimeStep):
        
        usedLDD = self.lddMap
                
        # ADDED: channel LDD -- DOES NOT WORK PROPERLY WITH waterBodyIds!!
        #---------------------------------------------------------------------
        #self.channelLDD = pcr.ifthenelse(self.WaterBodies.waterBodyIds != 0,\
        #                                    pcr.ldd(5),self.lddMap)
        #---------------------------------------------------------------------
        ...
        # ADDED: kinematic wave routing stuff
        #---------------------------------------------------------------------
        # initialization of wetted area, alpha and flow velocity values
        self.wettedArea,self.alphaQ = self.kinematicAlpha(self.channelStorage)
        self.flowVelocity= pcr.ifthenelse(self.wettedArea > 0,self.Q/self.wettedArea,0.)
        
        # definition of sub-loop for routing scheme - explicit scheme has to satisfy Courant condition
        # (copied from DynRout script of Rens)
        timeLimit = pcr.cellvalue(pcr.mapminimum((pcr.cover(pcr.ifthen(self.WaterBodies.waterBodyIds == 0,\
        ...
        nrIter = int(min(nrIter,self.timeSec/300.))
        ...
        for nrICur in range(nrIter):
            # function 'kinematic' as in DynRout:
            
            # initialize discharge for current sub-timestep
            #   with 'self.WaterBodies.waterBodyIds':
            #estQ = pcr.ifthenelse((self.channelStorage > 0.) & (self.WaterBodies.waterBodyIds == 0) ,\
            #                (self.wettedArea/self.alphaQ)**(1./self.betaQ),0.)
            #   without 'self.WaterBodies.waterBodyIds':
            estQ = pcr.ifthenelse((self.channelStorage > 0.),\
                            (self.wettedArea/self.alphaQ)**(1./self.betaQ),0.)
            
            # kinematic wave
            # self.Q = pcr.kinematic(ldd, Qold, q, alpha, beta, nrTimeSlices, dT, dX)
            self.Q = pcr.spatial(pcr.scalar(0.))
            #   with 'self.WaterBodies.waterBodyIds' and "ifthenelse-loop":
            #self.Q = pcr.ifthenelse(self.WaterBodies.waterBodyIds == 0,\
            #            pcr.kinematic(self.channelLDD,estQ,0.,self.alphaQ,\
            #                self.betaQ,1,deltaTime,self.channelLength),self.Q)
            #   without 'self.WaterBodies.waterBodyIds' and "ifthenelse-loop":
            self.Q = pcr.kinematic(usedLDD,estQ,0.,self.alphaQ,self.betaQ,1,\
                                    deltaTime,self.channelLength)
            
            # cover operation used in accuTravelTime, not in DynRout...
            # use here as well or not?
            self.Q = pcr.cover(self.Q, 0.0)
            
            # water bodies?
            # ...
            
            # update of channelStorage
            self.channelStorage += (-self.Q + pcr.upstream(usedLDD,self.Q))*deltaTime
            self.channelStorage = pcr.max(0.,self.channelStorage)
            
            # update alpha, wetted area, flow velocity
            self.wettedArea,self.alphaQ = self.kinematicAlpha(self.channelStorage)
            # Why this second update of wettedArea? see also .txt file notes
            #self.wettedArea= self.waterBodies.returnMapValue(self.wettedArea,\
            #    self.waterBodies.channelWidth+2.*self.waterBodies.updateWaterHeight())
            ...
            # functions 'kinwavestate' and 'kinwaveflux'
            
            # channel storage as input for previous state
            # with cover operation as used in 'accuTravelTime':
            #channelStorageForKinematicWave = self.channelStorage
            #channelStorageForKinematicWave = pcr.cover(channelStorageForKinematicWave,0.0)
            
            #newstate,newflux = kinwavestate,kinwaveflux(ldd,prevState,qChan,
            #                   alpha,beta,nrTimeSlices,timestepInSeconds,dX)
            # new flux [m3/s]
            #self.Q = pcr.kinwaveflux(usedLDD,channelStorageForKinematicWave,\
            #            0.,self.alphaQ,self.betaQ,1,deltaTime,self.channelLength)
            # cover operation as used in 'accuTravelTime':
            #self.Q = pcr.cover(self.Q, 0.0)
            # new state [m3]
            #self.channelStorage = pcr.kinwavestate(usedLDD,channelStorageForKinematicWave,\
            #                        0.,self.alphaQ,self.betaQ,1,deltaTime,self.channelLength)
            
            # update alpha, wetted area, flow velocity (see comments above)
            #self.wettedArea,self.alphaQ = self.kinematicAlpha(self.channelStorage)
            #self.flowVelocity= pcr.ifthenelse(self.wettedArea > 0,self.Q/self.wettedArea,0.)
        #---------------------------------------------------------------------
        ...
        # CHANGED: discharge from calculation of Q (output is already in m3/s)
        #---------------------------------------------------------------------
        # channel discharge (m3/s):
        self.discharge = self.Q
        #---------------------------------------------------------------------
        self.discharge = pcr.max(0., self.discharge)                   # reported channel discharge cannot be negative
        self.discharge = pcr.ifthen(self.landmask, self.discharge)
