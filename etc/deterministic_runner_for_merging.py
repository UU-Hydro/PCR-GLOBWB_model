...
class DeterministicRunner(DynamicModel):
    ...
    def dynamic(self):
        ...
        # at the last day of the year, merging pcraster maps
        if self.modelTime.isLastDayOfYear():

            # wait until all pcrglobwb model runs are done
            all_pcrglobwb_runs_are_ready = False
            self.count_check = 0
            while all_pcrglobwb_runs_are_ready == False:
                if datetime.datetime.now().second == 7 or\
                   ...
                   all_pcrglobwb_runs_are_ready = self.check_pcrglobwb_status()
