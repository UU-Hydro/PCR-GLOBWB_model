...
class GroundwaterModflow(object):
    ...
    def get_initial_heads(self):
        ...
        if self.iniItems.modflowTransientInputOptions['usingPredefinedInitialHead'] == "True": 
            ...
            if self.iniItems.online_coupling_between_pcrglobwb_and_moflow:
            
                ###########################################################################################################################################                                           
                msg  = "Initial conditions have been estimated from the steady state simulation using the input as defined in the ini/configuration file, "
                msg += "particularly from the section 'modflowSteadyStateInputOptions'. "
                logger.info(msg)
                
                steady_state_output_folder = self.iniItems..globalOptions['outputDir'] + "/steady-state/" + "states/"
                timeStamp = self.iniItems.globalOptions['startTime']
                ...
                for i in range(1, self.number_of_layers+1):
                    ...
                    # filename 
                    variable = var_name
                    filename = steady_state_output_folder + "/" +\
                               str(variable) + "_" +
                               timeStamp + ".map"
                    msg = "Reading from the file :" + str(filename)
                    logger.info(msg)

                    vars(self)[var_name] = vos.readPCRmapClone(filename, \
                                                               self.cloneMap, 
                                                               self.tmpDir, 
                                                               self.inputDir)
                    vars(self)[var_name] = pcr.cover(vars(self)[var_name], 0.0)                                           
