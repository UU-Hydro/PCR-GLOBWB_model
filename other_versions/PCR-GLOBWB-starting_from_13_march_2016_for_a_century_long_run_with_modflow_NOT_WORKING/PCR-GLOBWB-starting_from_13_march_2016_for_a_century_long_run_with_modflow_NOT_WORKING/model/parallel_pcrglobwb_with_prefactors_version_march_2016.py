...
online_coupling_between_pcrglobwb_and_moflow = False
# command lines to run a steady state of MODFLOW
if ("globalModflowOptions" in generalConfiguration.allSections) and\
   ...
   (generalConfiguration.modflowTransientInputOptions['usingPredefinedInitialHead'] == "False"):
    ...
    cmd  = "python deterministic_runner_for_monthly_merging_and_modflow.py " + iniFileName +" "+\
                                                                               debug_option +" steady-state-only "
    # execute MODFLOW
...
for clone_code in clone_codes:
   ...
   cmd += "python deterministic_runner_glue_with_parallel_option_version_march_2016.py " + iniFileName +" "+\
                                                                                           debug_option +" "+\
                                                                                           clone_code +" "
   cmd = cmd+" & "
   i_clone += 1
...
if online_coupling_between_pcrglobwb_and_moflow:       
