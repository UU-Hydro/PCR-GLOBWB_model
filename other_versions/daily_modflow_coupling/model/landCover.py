...
class LandCover(object):
    ...
    def updateLC(self, meteo, groundwater, routing,\
        ...
        # from the routing module, transfer floodVolume (m3) to the land surface part
        # - add/transfer the flood inundation water to topWaterLayer; flood volume in depth (m), per entire cell area
        transferFlood = routing.transferVolToTopWaterLayer / routing.cellArea
        # - add flood water to topWaterLayer
        self.topWaterLayer += transferFlood


        # calculate total PotET (based on meteo and cropKC)
        self.getPotET(meteo, currTimeStep)
        #
        # PS: Currently, the potential evaporation does not depend on water inundation extent. TODO: Calculate separately the land and open water potential evaporation. The current main difficulty is to distinguish the inundation over paddy fields and the one from riverine floods. The inundation for the former should cover the entire land, while the former usually cover only some minor part of land. 
    ...
    def getPotET_NOT_WORKING(self, meteo, routing, currTimeStep):

        # get crop coefficient for the land part:
        cropKC = pcr.cover(
                 vos.netcdf2PCRobjClone(self.cropCoefficientNC,'kc', \
                                    currTimeStep.fulldate, useDoy = 'daily_seasonal',\
                                    cloneMapFileName = self.cloneMap), 0.0)
        cropKC = pcr.ifthen(self.landmask, cropKC)
        self.inputCropKC = cropKC                                               # This line is needed for debugging. (Can we remove this? NOT NOW.)
        self.cropKC = pcr.max(cropKC, self.minCropKC)


        # get crop coefficient for the water part: 
        #
        if routing.no_zero_crop_water_coefficient == False: self.waterKC = 0.0
        #
        # NOTE: The netcdf file for this MUST BE AT MONTHLY resolution. TODO: FIXME
        # 
        if (currTimeStep.day == 1) or (currTimeStep.timeStepPCR == 1) and self.no_zero_crop_water_coefficient:
            waterKC = vos.netcdf2PCRobjClone(routing.fileCropKC,'kc', \
                               currTimeStep.fulldate, useDoy = 'month',\
                                       cloneMapFileName = self.cloneMap)
            self.waterKC = pcr.ifthen(self.landmask,\
                           pcr.cover(waterKC, 0.0))
            self.waterKC = pcr.max(routing.minCropWaterKC, self.waterKC)

        
        # calculate total potential ET (unit: m/day)
        self.totalPotET = pcr.ifthen(self.landmask,\
                                     self.cropKC  * meteo.referencePotET * self.land_fraction +\
                                     self.waterKC * meteo.referencePotET * self.water_fraction)


        # potential bare soil evaporation and transpiration (unit: m/day)
        self.potBareSoilEvap  = pcr.ifthen(self.landmask,\
                                self.minCropKC * meteo.referencePotET) * self.land_fraction
        self.potTranspiration = pcr.max(0.0, \
                                pcr.ifthen(self.landmask,\
                                self.totalPotET - self.potBareSoilEvap)) * self.land_fraction
    
        # potential open water evaporation (unit: m/day)
        self.potOpenWaterEvap = pcr.ifthen(self.landmask,\
                                self.waterKC * meteo.referencePotET * self.water_fraction)
        
        
        if self.debugWaterBalance:
            vos.waterBalanceCheck([self.totalPotET],\
                                  [self.potBareSoilEvap, self.potTranspiration, self.potOpenWaterEvap],\
                                  [],\
                                  [],\
                                  'partitioning potential evaporation',\
                                  True,\
    ...
    def interceptionUpdate(self, meteo, currTimeStep):
        ...
        if self.interceptionModuleType == 'Modified': 
            # extended interception definition/scope (not only canopy)
            self.potInterceptionFlux = self.totalPotET                   # this option is added by Edwin to extend the interception scope/definition
        ...
        if self.interceptionModuleType == 'Modified':
            #
            # fractions of potential bare soil evaporation and potential transpiration
            fracPotBareSoilEvap  = pcr.max(0.0, pcr.min(1.0, \
                                   vos.getValDivZero(self.potBareSoilEvap, \
                                                     self.potBareSoilEvap + self.potTranspiration + self.potOpenWaterEvap, vos.smallNumber)))
            fracPotTranspiration = pcr.max(0.0, 1.0 - fracPotBareSoilEvap)
            #
...
#~ 
    ...
    def calculateDirectRunoff(self):
        ...
        # amount of direct runoff is limited to available water in topWaterLayer
    ...
    def calculateOpenWaterEvap(self):
        ...
        # open water evaporation from paddy fields only
        if self.name == 'irrPaddy' or self.name == "irr_paddy":
            self.openWaterEvap = \
             pcr.min(\
             pcr.max(0.,self.topWaterLayer), remainingPotETP)  

        #~ # open water evaporation from paddy field and riverine flood - Note that this is still experimental.
        #~ self.openWaterEvap = \
             #~ pcr.min(\
             #~ pcr.max(0.,self.topWaterLayer), remainingPotETP)  
