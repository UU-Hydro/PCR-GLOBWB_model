...
class Routing(object):
    ...
    def getState(self):
        ...
        # The following variable is needed only for kinematic wave methods (i.e. kinematicWave  and simplifiedKinematicWave)
    ...
    def __init__(self,iniItems,initialConditions,lddMap):
        ...
        if 'minimumChannelGradient' in iniItems.routingOptions.keys():
            minimumChannelGradient = float(iniItems.routingOptions['minimumChannelGradient'])
        else:
    ...
    def getICs(self,iniItems,iniConditions = None):
        ...
        if iniConditions == None:
            ...
            # needed for kinematic wave method                                                                            
        ...
        # make sure that readAvlChannelStorage is limited to channelStorage
    ...
    def update(self,landSurface,groundwater,currTimeStep,meteo):
        ...
        # outgoing volume (Q at m3) at pits
        self.outgoing_volume_at_pits = pcr.ifthen(self.landmask,
                                       pcr.cover(
                                       pcr.ifthen(self.lddMap == pcr.ldd(5), self.Q), 0.0))
        # TODO: We have to track/accumulate water in endorheic basins. This is too DIFFICULT.
        # - An alternative solution is just assuming that all water leaving endorheic basin pits will evaporate.  


        # infiltration from surface water bodies (rivers/channels, as well as lakes and/or reservoirs) to groundwater bodies
        # - this exchange fluxes will be handed (to the groundwater) in the next time step
        self.calculate_exchange_to_groundwater(groundwater, currTimeStep)
        # TODO: For kinematic wave update, shall we move this process within the loop?

                
        # riverine flood volume (m3) to the land surface part
        ...
        # 
        # transfer riverine flood volume (m3) to the land surface part - this is still experimental
        self.transferVolToTopWaterLayer = 0.0
        if self.transferFloodInundationVolumeToTopWaterLayer:
            self.transferVolToTopWaterLayer = self.floodInundationVolume
        # - this exchange fluxes will be handed to the land surface in the next time step
        self.channelStorage = self.channelStorage - self.transferVolToTopWaterLayer
        ...
        # for debugging only
        self.negative_channel_storage_in_meter = pcr.min(0.0, self.channelStorage) / self.cellArea

        
        # old-style reporting (we need this one for debugging process)                            
    ...
    def calculate_exchange_to_groundwater(self, groundwater, currTimeStep):
        ...
        # This infiltration fluxes will be handed to groundwater in the next time step.

        if groundwater.useMODFLOW:
        
            # Basically, river bed exchange (infiltration) fluxes can be calculated via the RIV package of MODFLOW.
            # Yet, the RIV package often results negative channel storage. Therefore, the river bed exchange (infiltration) is calculated here.
            # This infiltration fluxes will be handed to groundwater in the next time step.

            # the default stress period is monthly
            stress_period = "monthly"
            if groundwater.coupleToDailyMODFLOW: stress_period = "daily"
 
            # groundwater head from the uppermost layer
            goundwater_head_elevation = groundwater.gw_modflow.getState()['groundwaterHeadLayer' + str(groundwater.gw_modflow.number_of_layers)]

            # surface water elevation values and etc
            surface_water_head_elevation, surface_water_bed_elevation_used, surface_water_bed_conductance = \
              groundwater.gw_modflow.set_river_package(discharge = None, 
                                                       channel_storage = pcr.max(0.0, self.channelStorage), 
                                                       currTimeStep = currTimeStep, 
                                                       simulation_type = "transient", 
                                                       exclude_river_infiltration = False, \
                                                       stress_period = stress_period, \
                                                       estimate_river_water_level_from_discharge = False, \
                                                       dynamicFracWat = self.dynamicFracWat, \
                                                       bankfullDepth = self.channelDepth, \
                                                       floodDepth = self.floodDepth,\
                                                       set_the_modflow_river_package = False)

            # surface water infiltration to groundwater in volume (m3)
            volumeSurfaceWaterInf = surface_water_bed_conductance * \
                                    pcr.ifthenelse(surface_water_head_elevation > goundwater_head_elevation, \
                                                   surface_water_head_elevation - pcr.max(goundwater_head_elevation, surface_water_bed_elevation_used), 0.0)
            volumeSurfaceWaterInf = pcr.max(0.0, volumeSurfaceWaterInf)
            
            # limited surface water infiltration based on river bed conductivity
            maximum_infiltration_volume = groundwater.riverBedConductivity * self.dynamicFracWat * self.cellArea
            volumeSurfaceWaterInf  = pcr.min(maximum_infiltration_volume, volumeSurfaceWaterInf)
            
            # limited surface water infiltration based on available water
            available_water_volume = pcr.max(0.0, self.channelStorage)
            volumeSurfaceWaterInf  = pcr.min(available_water_volume, volumeSurfaceWaterInf)
            
            #~ # surface water infiltration only occurs if baseflow has ceased - THIS IS NOT NEEDED
            #~ volumeSurfaceWaterInf  = pcr.ifthenelse(groundwater.baseflow > 0.0, 0.0, volumeSurfaceWaterInf)
            
            self.riverbedExchange  = pcr.max(0.0, volumeSurfaceWaterInf)

        else:
        
            # for runs without MODFLOW:
            # - current implementation based on the following principle
        ...
        self.riverbedExchange = pcr.ifthenelse(self.channelStorage > 0.0, pcr.min(self.riverbedExchange, pcr.max(0.0,self.channelStorage)), 0.0)
        self.riverbedExchange = pcr.cover(self.riverbedExchange, 0.0)
        self.riverbedExchange = pcr.max(0.0, self.riverbedExchange)
        self.riverbedExchange = pcr.min(self.riverbedExchange, pcr.max(0.0,self.channelStorage))
        self.riverbedExchange = pcr.ifthen(self.landmask, self.riverbedExchange)
            
        # update channelStorage (m3) after riverbedExchange (m3)
        self.channelStorage = self.channelStorage - self.riverbedExchange
        self.local_input_to_surface_water = self.local_input_to_surface_water - self.riverbedExchange


        self.riverbedExchangeInMeter = self.riverbedExchange / self.cellArea
    ...
    def returnInundationFractionAndFloodDepth(self, channelStorage):
        ...
        if self.floodPlain:
            ...
            floodDepth  = pcr.ifthenelse(inundatedFraction > 0., \
                          excessVolume/(pcr.max(self.min_fracwat_for_water_height, inundatedFraction)*self.cellArea), 0.0)  # unit: m
            #~ # - maximum flood depth
