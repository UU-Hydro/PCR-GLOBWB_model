...
class Reporting(object):
    ...
    def additional_post_processing(self):
        ...
        #-RvB: specific reporting for bogs, mires and fens: 
        # persistent fraction saturated area, water depth and runoff depth as approximation of wetlands
        # persistentSaturatedFraction,pondedWaterDepth,runoffDepthSaturatedFraction
        self.persistentSaturatedFraction= pcr.scalar(0)
        self.pondedWaterDepth= pcr.scalar(0)
        self.runoffDepthSaturatedFraction= pcr.scalar(0)
        self.runoffDepthOther= self.runoff

        #-select only natural types
        coverTypes= self._model.landSurface.landCoverObj.keys()
        for coverType in coverTypes:
					if 'Paddy' in coverType: 
						coverTypes.remove(coverType)
					elif 'crop' in coverType:
						coverTypes.remove(coverType)

        for coverType in coverTypes:
					#-area saturated
					if isinstance(self._model.landSurface.landCoverObj[coverType].satAreaFracOld,NoneType):
						satAreaContribution= self._model.landSurface.landCoverObj[coverType].fracVegCover * \
							self._model.landSurface.landCoverObj[coverType].satAreaFrac
					else:
						satAreaContribution= self._model.landSurface.landCoverObj[coverType].fracVegCover * \
							pcr.min(self._model.landSurface.landCoverObj[coverType].satAreaFrac,\
								self._model.landSurface.landCoverObj[coverType].satAreaFracOld)
					# persistentSaturatedFraction
					self.persistentSaturatedFraction+= satAreaContribution
					# pondedWaterDepth
					self.pondedWaterDepth+= satAreaContribution * \
						0.5*pcr.max(0,self._model.landSurface.landCoverObj[coverType].soilWaterStorage-\
							self._model.landSurface.landCoverObj[coverType].rootZoneWaterStorageMin)					
					# runoff depth
					self.runoffDepthSaturatedFraction= satAreaContribution *\
						self._model.landSurface.landCoverObj[coverType].directRunoff
				#-runoff depth over other areas than natural, persistent flooded
        self.runoffDepthOther-= self.runoffDepthSaturatedFraction
        self.runoffDepthOther= pcr.ifthenelse(self.persistentSaturatedFraction < 1.0,\
						pcr.max(0,self.runoffDepthOther)/pcr.max(1.e-9,1.-self.persistentSaturatedFraction),0)
				#-update with area
        self.pondedWaterDepth/= pcr.max(1.e-9,self.persistentSaturatedFraction)
        self.runoffDepthSaturatedFraction/= pcr.max(1.e-9,self.persistentSaturatedFraction)
        self.persistentSaturatedFraction= pcr.ifthenelse(self.persistentSaturatedFraction >= 1.e-9,\
					self.persistentSaturatedFraction,0)
        self.pondedWaterDepth= pcr.ifthenelse(self.persistentSaturatedFraction >= 1.e-9,\
					self.pondedWaterDepth,0)
        self.runoffDepthSaturatedFraction= pcr.ifthenelse(self.persistentSaturatedFraction >= 1.e-9,\
					self.runoffDepthSaturatedFraction,0)
        #-processing for bogs, mires and fens completed				
...
#~ 
        #~ # inundation depth (m)
        #~ if 'water_height' in vars(self._model.routing).keys():
					#~ self.InundationDepth = self._model.routing.water_height
        #~ else: 
					#~ self.InundationDepth=pcr.scalar(0)
					#~ 
				        #~ 
        #~ 
        #~ # Some examples to report variables from certain land cover types:
...
#~ 
        ...
        #~ ######################################################################################################################################################################
#~ 
        #~ #actualET from each land cover type - not multiplied by fracvegcover!
        #~ if self._model.landSurface.noLandCoverFractionCorrection: 
            #~ self.actualET_irrPaddy = self._model.landSurface.landCoverObj['irrPaddy'].actualET# * self._model.landSurface.landCoverObj['irrPaddy'].fracVegCover
            #~ self.actualET_irrNonPaddy = self._model.landSurface.landCoverObj['irrNonPaddy'].actualET# * self._model.landSurface.landCoverObj['irrNonPaddy'].fracVegCover
            #~ self.actualET_tall_natural = self._model.landSurface.landCoverObj['tall_natural'].actualET# * self._model.landSurface.landCoverObj['tall_natural'].fracVegCover
            #~ self.actualET_short_natural = self._model.landSurface.landCoverObj['short_natural'].actualET# * self._model.landSurface.landCoverObj['short_natural'].fracVegCover
            #~ self.actualET_rainfed_crops = self._model.landSurface.landCoverObj['rainfed_crops'].actualET# * self._model.landSurface.landCoverObj['rainfed_crops'].fracVegCover
            #~ self.actualET_pasture = self._model.landSurface.landCoverObj['pasture'].actualET# * self._model.landSurface.landCoverObj['pasture'].fracVegCover
#~ 
        #~ #actTranspiTotal from each land cover type - not multiplied by fracvegcover
        #~ if self._model.landSurface.noLandCoverFractionCorrection:  
            #~ self.actTranspi_irrPaddy = self._model.landSurface.landCoverObj['irrPaddy'].actTranspiTotal# * self._model.landSurface.landCoverObj['irrPaddy'].fracVegCover
            #~ self.actTranspi_irrNonPaddy = self._model.landSurface.landCoverObj['irrPaddy'].actTranspiTotal# * self._model.landSurface.landCoverObj['irrPaddy'].fracVegCover
            #~ self.actTranspi_tall_natural = self._model.landSurface.landCoverObj['tall_natural'].actTranspiTotal# * self._model.landSurface.landCoverObj['tall_natural'].fracVegCover
            #~ self.actTranspi_short_natural = self._model.landSurface.landCoverObj['short_natural'].actTranspiTotal# * self._model.landSurface.landCoverObj['short_natural'].fracVegCover
            #~ self.actTranspi_rainfed_crops = self._model.landSurface.landCoverObj['rainfed_crops'].actTranspiTotal# * self._model.landSurface.landCoverObj['rainfed_crops'].fracVegCover
            #~ self.actTranspi_pasture = self._model.landSurface.landCoverObj['pasture'].actTranspiTotal# * self._model.landSurface.landCoverObj['pasture'].fracVegCover
#~ 
    #~ def report(self):
#~ 
        #~ # recap all variables
