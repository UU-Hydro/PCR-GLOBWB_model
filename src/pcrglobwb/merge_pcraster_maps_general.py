...
import numpy as np
from pcraster import Scalar, numpy2pcr, pcr2numpy, readmap, report, setclone
...
def checkResolution(c1, c2):
    ...
    c2 = round(c2, nd)
    if c1 != c2:
        print("resolutions %s, %s differ" % (s1, s2))
    return c1 == c2, nd
...
if sys.argv[6] == "defined":
    cellsize_in_arcsec = float(sys.argv[7])
    xmin = float(sys.argv[8])
    ymin = float(sys.argv[9])
    xmax = float(sys.argv[10])
    ymax = float(sys.argv[11])
    deltaLat = float(sys.argv[7]) / (3600.0)
    deltaLon = deltaLat
    lonMin = xmin
    latMin = ymin
    lonMax = xmax
