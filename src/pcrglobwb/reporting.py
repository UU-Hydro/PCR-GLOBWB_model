...
class Reporting(object):
    ...
    def __init__(self, configuration, model, modelTime):
        ...
        self.offlineRun = configuration.routingOptions[
            "offlineRun"
        ]  # for offline DynQual runs
        self.quality = configuration.routingOptions[
            "quality"
        ]  # for water quality modelling
        self.calculateLoads = configuration.routingOptions[
            "calculateLoads"
        ]  # for calculating pollutant loads in model run
        self.loadsPerSector = configuration.routingOptions[
            "loadsPerSector"
        ]  # for calculating pollutant loads per sector
        ...
        if self.configuration.debug_to_version_one:
    ...
    def post_processing(self):
        ...
        if self.debug_to_version_one:
            if self._modelTime.timeStepPCR == 1:
                self.report_static_maps_for_debugging()
    ...
    def report_forcing_for_debugging(self):
        ...
        if self._modelTime.timeStepPCR == 1:
            self.directory_for_forcing_maps = vos.getFullPath(
                "meteo/", self.configuration.mapsDir
            )
            if os.path.exists(self.directory_for_forcing_maps):
                shutil.rmtree(self.directory_for_forcing_maps)
            os.makedirs(self.directory_for_forcing_maps)

        # writing precipitation time series maps
        file_name = (
            self.directory_for_forcing_maps
            + pcr.framework.frameworkBase.generateNameT(
                "/" + varDicts.pcr_short_name["precipitation"],
                self._modelTime.timeStepPCR,
            )
        )
        ...
        file_name = (
            self.directory_for_forcing_maps
            + pcr.framework.frameworkBase.generateNameT(
                "/" + varDicts.pcr_short_name["temperature"],
                self._modelTime.timeStepPCR,
            )
        )
        ...
        file_name = (
            self.directory_for_forcing_maps
            + pcr.framework.frameworkBase.generateNameT(
                "/" + varDicts.pcr_short_name["referencePotET"],
                self._modelTime.timeStepPCR,
            )
        )
    ...
    def report_vegetation_phenology_for_debugging(self):
        ...
        # prepare directory
        if self._modelTime.timeStepPCR == 1:
            self.directory_for_cover_fraction_maps = vos.getFullPath(
                "cover_fraction/", self.configuration.mapsDir
            )
            if os.path.exists(self.directory_for_cover_fraction_maps):
                shutil.rmtree(self.directory_for_cover_fraction_maps)
            os.makedirs(self.directory_for_cover_fraction_maps)

        # writing CF_SHORTSTACK maps
        file_name = (
            self.directory_for_cover_fraction_maps
            + pcr.framework.frameworkBase.generateNameT(
                "/cv_s", self._modelTime.timeStepPCR
            )
        )
        pcr.report(
            self._model.landSurface.landCoverObj["grassland"].coverFraction, file_name
        )
        ...
        file_name = (
            self.directory_for_cover_fraction_maps
            + pcr.framework.frameworkBase.generateNameT(
                "/cv_t", self._modelTime.timeStepPCR
            )
        )
        pcr.report(
            self._model.landSurface.landCoverObj["forest"].coverFraction, file_name
        )
        ...
        # prepare directory
        if self._modelTime.timeStepPCR == 1:
            self.directory_for_interception_capacity_input_maps = vos.getFullPath(
                "interception_capacity_input/", self.configuration.mapsDir
            )
            if os.path.exists(self.directory_for_interception_capacity_input_maps):
        ...
        file_name = (
            ...
            + pcr.framework.frameworkBase.generateNameT(
                "/smax_s", self._modelTime.timeStepPCR
            )
        )
        pcr.report(
            self._model.landSurface.landCoverObj["grassland"].interceptCapInput,
            file_name,
        )
        ...
        file_name = (
            self.directory_for_interception_capacity_input_maps
            + pcr.framework.frameworkBase.generateNameT(
                "/smax_t", self._modelTime.timeStepPCR
            )
        )
        pcr.report(
            self._model.landSurface.landCoverObj["forest"].interceptCapInput, file_name
        )
        ...
        # prepare directory
        if self._modelTime.timeStepPCR == 1:
            self.directory_for_crop_coefficient_maps = vos.getFullPath(
                "crop_coefficient/", self.configuration.mapsDir
            )
            if os.path.exists(self.directory_for_crop_coefficient_maps):
        ...
        file_name = (
            ...
            + pcr.framework.frameworkBase.generateNameT(
                "/kc_s", self._modelTime.timeStepPCR
            )
        )
        pcr.report(
            self._model.landSurface.landCoverObj["grassland"].inputCropKC, file_name
        )
        ...
        file_name = (
            self.directory_for_crop_coefficient_maps
            + pcr.framework.frameworkBase.generateNameT(
                "/kc_t", self._modelTime.timeStepPCR
            )
        )
        pcr.report(
            self._model.landSurface.landCoverObj["forest"].inputCropKC, file_name
        )
    ...
    def report_static_maps_for_debugging(self):
        ...
        pcr.report(
            self._model.routing.landmask, self.configuration.mapsDir + "/catclone.map"
        )
        pcr.report(
            self._model.routing.cellArea, self.configuration.mapsDir + "/cellarea30.map"
        )
        ...
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].slopeLength,
            self.configuration.mapsDir + "/globalbcat.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].tanslope,
            self.configuration.mapsDir + "/globalgradslope.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].orographyBeta,
            self.configuration.mapsDir + "/globalboro.map",
        )

        # LDD      = maps\lddsound_30min.map;                              # local drainage direction map
        pcr.report(
            self._model.routing.lddMap,
            self.configuration.mapsDir + "/lddsound_30min.map",
        )
        ...
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0001,
            self.configuration.mapsDir + "/hydro1k_dzrel0001.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0005,
            self.configuration.mapsDir + "/hydro1k_dzrel0005.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0010,
            self.configuration.mapsDir + "/hydro1k_dzrel0010.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0020,
            self.configuration.mapsDir + "/hydro1k_dzrel0020.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0030,
            self.configuration.mapsDir + "/hydro1k_dzrel0030.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0040,
            self.configuration.mapsDir + "/hydro1k_dzrel0040.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0050,
            self.configuration.mapsDir + "/hydro1k_dzrel0050.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0060,
            self.configuration.mapsDir + "/hydro1k_dzrel0060.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0070,
            self.configuration.mapsDir + "/hydro1k_dzrel0070.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0080,
            self.configuration.mapsDir + "/hydro1k_dzrel0080.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0090,
            self.configuration.mapsDir + "/hydro1k_dzrel0090.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].dzRel0100,
            self.configuration.mapsDir + "/hydro1k_dzrel0100.map",
        )
        ...
        for coverType in ["forest", "grassland"]:
            pcr.report(
                self._model.landSurface.landCoverObj[coverType].fracVegCover,
                self.configuration.mapsDir
                + "/vegf_"
                + version_one_cover_type[coverType]
                + ".map",
            )
        ...
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].satVolMoistContUpp,
            self.configuration.mapsDir + "/fao30_ths30.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].satVolMoistContLow,
            self.configuration.mapsDir + "/fao30_ths100.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].resVolMoistContUpp,
            self.configuration.mapsDir + "/fao30_thr30.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].resVolMoistContLow,
            self.configuration.mapsDir + "/fao30_thr100.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].airEntryValueUpp,
            self.configuration.mapsDir + "/fao30_psis30.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].airEntryValueLow,
            self.configuration.mapsDir + "/fao30_psis100.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].poreSizeBetaUpp,
            self.configuration.mapsDir + "/fao30_beta30.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].poreSizeBetaLow,
            self.configuration.mapsDir + "/fao30_beta100.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].kSatUpp,
            self.configuration.mapsDir + "/fao30_ks30.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].kSatLow,
            self.configuration.mapsDir + "/fao30_ks100.map",
        )
        ...
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].thickUpp,
            self.configuration.mapsDir + "/fao30_z1_permafrost.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].thickLow,
            self.configuration.mapsDir + "/fao30_z2_permafrost.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].storCapUpp,
            self.configuration.mapsDir + "/fao30_sc1_permafrost.map",
        )
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].storCapLow,
            self.configuration.mapsDir + "/fao30_sc2_permafrost.map",
        )

        # WMAX          sv  maps\fao30_sc_permafrost.map              # WMAX        tv  maps\fao30_sc_permafrost.map
        pcr.report(
            self._model.landSurface.soil_topo_parameters[
                "default"
            ].rootZoneWaterStorageCap,
            self.configuration.mapsDir + "/fao30_sc_permafrost.map",
        )

        # P2_IMP        sv  maps\fao30_p2imp_permafrost.map           # P2_IMP      tv  maps\fao30_p2imp_permafrost.map
        pcr.report(
            self._model.landSurface.soil_topo_parameters["default"].percolationImp,
            self.configuration.mapsDir + "/fao30_p2imp_permafrost.map",
        )
        ...
        for coverType in ["forest", "grassland"]:
            pcr.report(
                self._model.landSurface.landCoverObj[coverType].minSoilDepthFrac,
                self.configuration.mapsDir
                + "/minf_"
                + version_one_cover_type[coverType]
                + ".map",
            ...
            pcr.report(
                self._model.landSurface.landCoverObj[coverType].maxSoilDepthFrac,
                self.configuration.mapsDir
                + "/maxf_"
                + version_one_cover_type[coverType]
                + ".map",
            ...
            pcr.report(
                self._model.landSurface.landCoverObj[coverType].rootFraction1,
                self.configuration.mapsDir
                + "/rfrac1_"
                + version_one_cover_type[coverType]
                + ".map",
            ...
            pcr.report(
                self._model.landSurface.landCoverObj[coverType].rootFraction2,
                self.configuration.mapsDir
                + "/rfrac2_"
                + version_one_cover_type[coverType]
                + ".map",
        ...
        pcr.report(
            self._model.groundwater.recessionCoeff,
            self.configuration.mapsDir + "/globalalpha.map",
        )
        pcr.report(
            self._model.groundwater.specificYield,
            self.configuration.mapsDir + "/specificyield.map",
        )
    ...
    def basic_post_processing(self):
        ...
        if self._model.landSurface.numberOfSoilLayers == 3:
            ...
            self.satDegUppSurface = (
                self._model.landSurface.satDegUpp000005
            )  # unit: percentage
    ...
    def additional_post_processing(self):
        ...
        self.totalRunoff = (
            self.runoff + self.local_water_body_flux
        ...
        if self._model.routing.quality:
            ...
            )  # minimum demands in m3 s-1 (temperature-dependent technologies)
            self.powerplants_fw_q = pcr.ifthen(
                self._model.routing.landmask,
                self._model.landSurface.water_demand.water_demand_thermoelectric.powerplants_fw_q,
            )  # water temperature dependent demands in m3 s-1 (temperature-dependent technologies)
            self.PowTwload = pcr.ifthen(
                self._model.routing.landmask, self._model.routing.PowTwload
            )  # unrouted temperature loadings from powerplants in W (temperature-dependent technologies)
        ...
        if (
            ...
            if self._model.groundwater.useMODFLOW:
                ...
                self.groundwaterThicknessEstimate = pcr.ifthen(
                    self._model.routing.landmask,
                    self._model.groundwater.gw_modflow.storage_coefficient_1
                    * (
                        self._model.groundwater.groundwaterHeadLayer1
                        - self._model.groundwater.gw_modflow.bottom_layer_1
                    ),
                )
                ...
                if self._model.groundwater.gw_modflow.number_of_layers == 2:
                    self.groundwaterThicknessEstimate += pcr.ifthen(
                        ...
                        * (
                            self._model.groundwater.groundwaterHeadLayer2
                            - self._model.groundwater.gw_modflow.bottom_layer_2
                        ),
                    )
            else:
    ...
    def e2o_post_processing(self):
        ...
        self.ECanop = (
            -self._model.landSurface.interceptEvap / 86.4
        )  # report in kg m-2 s-1
        self.TVeg = (
            -self._model.landSurface.actTranspiTotal / 86.4
        )  # report in kg m-2 s-1
        self.ESoil = (
            -self._model.landSurface.actBareSoilEvap / 86.4
        )  # report in kg m-2 s-1
        self.EWater = (
        ...
        ) * 1000  # report in kg m-2
        self.SurfMoist = (
            self._model.landSurface.storUppTotal * 1000
        )  # report in kg m-2 (water in SurfLayerThick)
        self.RootMoist = (
            self._model.landSurface.storUppTotal + self._model.landSurface.storLowTotal
        ) * 1000  # report in kg m-2 (water in RootLayerThick)
        self.TotMoist = self.RootMoist  # equals RootMoist...
        self.GroundMoist = (
            self._model.groundwater.storGroundwater * 1000
        )  # self._model.groundwater. # report in kg m-2
