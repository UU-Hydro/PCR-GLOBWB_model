...
def getPsychrometricConstant(
    atmosphericPressure, cpAir, epsilon, latentHeatVaporization
):
...
"""
    ...
    return (
        ...
        * pcr.max(0, ea0 - eaFactor * eAct**0.5)
...
def updateShortWaveRadiation(shortWaveRadiation, albedo=0.00, fraction=1.00):
...
\n"""
    ...
    def __init__(
        ...
        rhoAir=1.2047,
        windHeight=2.00,
        temperatureHeight=2.00,
        albedo=0.23,
        canopyResistance=70.0,
...
\n"""
        ...
        Z0h = 0.1 * Z0m
        raTerm = (
            pcr.ln((self.windHeight - Zd) / (Z0m))
            * pcr.ln((self.temperatureHeight - Zd) / (Z0h))
            * (self.karmanConst) ** -2
        )
...
def main():
    ...
    for month in months:
        ...
        vapourPressure = 10.0 * pcr.readmap(
            pcrm.generateNameT(vapourPressureFileRoot, month)
        )
        shortWaveRadiation = pcr.readmap(
        ...
        longWaveRadiation = getLongWaveRadiation(
            temperature, vapourPressure, fractionShortWaveRadiation
        )
        pcr.report(
            shortWaveRadiation,
            os.path.join(outputPath, pcrm.generateNameT("rads", month)),
        )
        pcr.report(
            longWaveRadiation,
        ...
        pcr.report(
            penMonModel.potentialEvaporation,
