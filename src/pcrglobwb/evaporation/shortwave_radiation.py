...
"""
    ...
    return pcr.min(
        1.0,
        compute_sat_pressure(pcr.spatial(pcr.scalar(tdew)))
...
as a function of the day angle in radians.
 ...
 """
    ...
    solar_declination = (
        ...
        + 0.070257 * pcr.sin(day_angle)
        - 0.006758 * pcr.cos(2 * day_angle)
        + 0.000907 * pcr.sin(2 * day_angle)
        - 0.002697 * pcr.cos(3 * day_angle)
...
orbit around the sun as a function of the day angle in radians.
 ...
 """
    ...
    eccentricity = (
        ...
        + 0.00128 * pcr.sin(day_angle)
        + 0.000719 * pcr.cos(2 * day_angle)
...
def compute_radsw_ext(
    latitude, solar_declination, eccentricity, day_length, solar_constant=118.1
):
    """
...
solar_constant specified.
  ...
  """
    ...
    radsw_ext = (
        ...
        * eccentricity
        * (
            pcr.cos(solar_declination)
            * pcr.cos(latitude)
            * pcr.sin(0.5 * 0.2618 * day_length)
            / 0.2618
            + 0.5 * pcr.sin(solar_declination) * pcr.sin(latitude) * day_length
        )
    )
...
"""
    ...
    def __init__(
        self, latitude, elevation, temp_annual, delta_temp_mean, solar_constant=118.1
    ):
        """
...
"""
    ...
    def parameter_initialization(
        self, latitude, elevation, temp_annual, delta_temp_mean
    ):
        """
