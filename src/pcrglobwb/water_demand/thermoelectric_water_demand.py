...
class ThermoelectricWaterDemand(object):
    ...
    def __init__(self, iniItems, landmask):
        ...
        if iniItems.waterDemandOptions["includeThermoelectricWaterDemand"] == "True":
            ...
            logger.info(
                "Thermoelectric water demand is NOT included in the calculation."
            )

        if self.thermoelectricWaterDemandOption:
            self.thermoelectricWaterDemandFile = vos.getFullPath(
    ...
    def read_thermoelectric_water_demand_from_files(self, currTimeStep):
        ...
        if currTimeStep.timeStepPCR == 1 or currTimeStep.day == 1:
            if self.thermoelectricWaterDemandOption:
                ...
                self.thermoelectricNettoDemand = pcr.max(
                    0.0,
                    pcr.cover(
                        vos.netcdf2PCRobjClone(
                            ncFile=self.thermoelectricWaterDemandFile,
            ...
            self.thermoelectricNettoDemand = pcr.cover(
                self.thermoelectricNettoDemand, 0.0
            )
            self.thermoelectricNettoDemand = pcr.min(
    ...
    def readPowerplantData(self, currTimeStep, routing):
        ...
        self.powerplants_fwfixed_ratio = pcr.cover(self.powerplants_fwfixed_ratio, 0.0)

        # seawater plants
        self.powerplants_sw_capacity = vos.netcdf2PCRobjClone(
            routing.powerplants_swNC,
            "capacity",
            str(currTimeStep.fulldate),
            useDoy="yearly",
            cloneMapFileName=self.cloneMap,
    ...
    def calculatePowerplantDemands(self, currTimeStep, routing):
        ...
        self.powerplants_fw_qmin = (
            ...
            * 1e6
            * ((1 - self.powerplants_fw_ntotal) / self.powerplants_fw_nelec)
            * (
                (
                    (1 - self.powerplants_fw_alpha)
                    * (1 - self.powerplants_fw_beta)
                    * self.powerplants_fw_omega
                    * self.powerplants_fw_EZ
                )
        ...
        self.powerplants_fw_q = (
            ...
            * (
                ...
                / (
                    routing.densityWater
                    * routing.specificHeatWater
                    * self.min_Tlmax_dTlmax
                )
            )
        )
        ...
        self.thermoelectricNettoDemand = self.thermoelectricGrossDemand - pcr.cover(
            (self.powerplants_fw_rf + self.powerplants_fwfixed_rf)
            * 86400
            / routing.cellArea,
            0.0,
        )
