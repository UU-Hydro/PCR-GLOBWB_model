from __future__ import print_function

import datetime
import glob
import logging
import os
import shutil
import sys

from pcrglobwb import configuration
from pcrglobwb.common import virtualOS as vos

logger = logging.getLogger(__name__)

from pcrglobwb.common import disclaimer

disclaimer.print_disclaimer()

iniFileName = os.path.abspath(sys.argv[1])

# option for debugging and parallelization
debug_option = "parallel"
if len(sys.argv) > 2:
    debug_option = str(sys.argv[2])

generalConfiguration = configuration.Configuration(
    iniFileName=iniFileName, debug_mode=False, no_modification=False
)

clean_previous_output = True
if (
    generalConfiguration.globalOptions["cloneAreas"] == "part_one"
    or generalConfiguration.globalOptions["cloneAreas"] == "part_two"
):
    clean_previous_output = False

# clean existing files in the output directory (can be done for global runs)
if clean_previous_output and os.path.exists(
    generalConfiguration.globalOptions["outputDir"]
):
    shutil.rmtree(generalConfiguration.globalOptions["outputDir"])

generalOutputFolder = generalConfiguration.globalOptions["outputDir"]
logFileFolder = generalOutputFolder + "/global/log/"
if os.path.exists(logFileFolder) and clean_previous_output:
    shutil.rmtree(logFileFolder)
    os.makedirs(logFileFolder)
if os.path.exists(logFileFolder) == False:
    os.makedirs(logFileFolder)
generalConfiguration.initialize_logging(logFileFolder)

# copy the ini file to the log folder
timestamp = datetime.datetime.now()
logger.info("Copying ini file to the folder %s", logFileFolder)
shutil.copy(
    iniFileName,
    logFileFolder
    + os.path.basename(iniFileName)
    + "_"
    + str(timestamp.isoformat()).replace(":", ".")
    + ".ini",
)

# global output directory (its "maps" folder will contain the merged PCRaster maps)
global_maps_folder = generalConfiguration.globalOptions["outputDir"] + "/global/maps/"
if os.path.exists(global_maps_folder):
    shutil.rmtree(global_maps_folder)
    os.makedirs(global_maps_folder)
else:
    os.makedirs(global_maps_folder)

# back up the Python scripts to a backup folder and go there
scriptDir = generalConfiguration.globalOptions["outputDir"] + "/global/scripts/"
if (
    generalConfiguration.globalOptions["cloneAreas"] == "part_one"
    or generalConfiguration.globalOptions["cloneAreas"] == "part_two"
):
    scriptDir = scriptDir + "/" + generalConfiguration.globalOptions["cloneAreas"] + "/"
logger.info("Making the backup of the python scripts used to the folder %s", scriptDir)
if os.path.exists(scriptDir):
    shutil.rmtree(scriptDir)
os.makedirs(scriptDir)
path_of_this_module = os.path.abspath(os.path.dirname(__file__))
for filename in glob.glob(os.path.join(path_of_this_module, "*.py")):
    shutil.copy(filename, scriptDir)
os.chdir(scriptDir)


# option to include merging
with_merging_or_modflow = True
# merging can be skipped in the ini file (e.g. for runs with spin-up);


# for parallel runs with spin-up, merging and/or MODFLOW coupling is not possible
if float(generalConfiguration.globalOptions["maxSpinUpsInYears"]) > 0:
    msg = "The run is set with some spin-ups. We can NOT combine this with merging processes and modflow calculation. "
    logger.warning(msg)
    logger.warning(msg)
    logger.warning(msg)
    logger.warning(msg)
    logger.warning(msg)
    if (
        "with_merging" in list(generalConfiguration.globalOptions.keys())
        and generalConfiguration.globalOptions["with_merging"] == "False"
    ):
        with_merging_or_modflow = False
    else:
        msg = "You set this run (with spin-ups) either with modflow or merging processes. That is not possible."
        logger.warning(msg)
        sys.exit()


# PCR-GLOBWB clone areas (for multiple runs)
clone_codes = list(set(generalConfiguration.globalOptions["cloneAreas"].split(",")))
# one global run (on a fat node)
if clone_codes[0] == "Global":
    clone_codes = ["M%02d" % i for i in range(1, 54, 1)]
# two (thick) nodes
if clone_codes[0] == "part_one":
    # the relatively large clones
    clone_codes = [
        "M17",
        "M19",
        "M26",
        "M13",
        "M18",
        "M20",
        "M05",
        "M03",
        "M21",
        "M46",
        "M27",
        "M49",
        "M16",
        "M44",
        "M52",
        "M25",
        "M09",
        "M08",
        "M11",
        "M42",
        "M12",
        "M39",
    ]
if clone_codes[0] == "part_two":
    # the relatively small clones
    clone_codes = [
        "M07",
        "M15",
        "M38",
        "M48",
        "M40",
        "M41",
        "M22",
        "M14",
        "M23",
        "M51",
        "M04",
        "M06",
        "M10",
        "M02",
        "M45",
        "M35",
        "M47",
        "M50",
        "M24",
        "M01",
        "M36",
        "M53",
        "M33",
        "M43",
        "M34",
        "M37",
        "M31",
        "M32",
        "M28",
        "M30",
        "M29",
    ]
    # TODO: improve the selection of clones for part two, e.g. clones_part_two = clones_all - clones_part_one
    # merging and MODFLOW are run on another node
    with_merging_or_modflow = False


logger.info("Running transient PCR-GLOBWB with/without MODFLOW ")
i_clone = 0
cmd = ""
for clone_code in clone_codes:

    cmd += (
        "python deterministic_runner_glue_with_parallel_and_modflow_options.py "
        + iniFileName
        + " "
        + debug_option
        + " "
        + clone_code
        + " "
    )
    cmd = cmd + " & "
    i_clone += 1


# runs with spin-up should not be combined with MODFLOW


# command line(s) for merging and MODFLOW
if with_merging_or_modflow:

    logger.info("Also with merging and/or MODFLOW processes ")

    cmd += (
        "python deterministic_runner_for_monthly_modflow_and_merging.py "
        + iniFileName
        + " "
        + debug_option
        + " transient"
    )

    cmd = cmd + " & "


# wait for all background processes
cmd = cmd + "wait"

print(cmd)
msg = "Call: " + str(cmd)
logger.debug(msg)


vos.cmd_line(cmd, using_subprocess=False)
