...
class LandCover(object):
    ...
    def __init__(self,iniItems,nameOfSectionInIniFile,parameters,landmask,irrigationEfficiency,usingAllocSegments = False):
        ...
        if "maxSoilDepthFrac" in self.iniItemsLC.keys():
            if self.iniItemsLC['maxSoilDepthFrac'] != "None": self.minSoilDepthFrac = pcr.min(self.minSoilDepthFrac, self.maxSoilDepthFrac) 
        
        # avoid small values (in order to avoid rounding error)
        self.fracVegCover = pcr.cover(self.fracVegCover, 0.0)
        if "noLandCoverFractionCorrection" not in iniItems.landSurfaceOptions.keys() or\
           iniItems.landSurfaceOptions["noLandCoverFractionCorrection"] == "False": self.fracVegCover = pcr.rounddown(self.fracVegCover * 1000.)/1000.
    ...
    def calculateWaterDemand(self, parameters, \
        ...
        if self.name == 'irrPaddy' and self.includeIrrigation: self.potential_irrigation_loss += self.design_percolation_loss
        ...
        if self.name == 'irrPaddy' and self.includeIrrigation: self.potential_irrigation_loss = pcr.max(self.design_percolation_loss, self.potential_irrigation_loss)
    ...
    def scaleAllFluxesForIrrigatedAreas(self, parameters, groundwater):
        ...
        if self.name.startswith('irr') and self.includeIrrigation: self.interflow = 0.                                                                                                                                        
