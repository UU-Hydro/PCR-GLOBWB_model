...
def waterAbstractionAndAllocationFailedTry6Oct2017(water_demand_volume,
                                  available_water_volume, 
                                  allocation_zones,
                                  zone_area = None,
                                  ...
                                  then_prioritizing_maximum_source = True):

    # TODO: Refactor to remove the variables: high_volume_treshold (not used anymore), landmask (must always be defined), ignore_small_values (not used anymore)
    
    logger.debug("Allocation of abstraction.")
    
    # variables to accumulate allocation and abstraction fluxes over several steps
    cellAllocation  = pcr.scalar(0.0)
    cellAbstraction = pcr.scalar(0.0)
    
    # make sure that we have good input variables
    cellVolDemand   = pcr.ifthen(landmask, pcr.cover(pcr.max(0.0, water_demand_volume), 0.0))
    cellAvlWater    = pcr.ifthen(landmask, pcr.cover(pcr.max(0.0, available_water_volume), 0.0))
    allocationZones = pcr.ifthen(landmask, allocation_zones)
    ...
    if prioritizing_local_source:
        ...
        cellAllocation  = pcr.max(0.0, pcr.min(cellVolDemand, cellAvlWater))
        cellAbstraction = cellAllocation * 1.0
    ...
    #~ # satistify demand with cells that have highest sources:
    #~ if then_prioritizing_maximum_source:
        #~ logger.debug("Allocation of abstraction - satisfy demand with maximum source within the allocation zone.")
#~ 
        #~ # remaining total demand volume in each zone/segment (unit: m3)
        #~ zoneVolDemand = pcr.areatotal(cellVolDemand, allocationZones)
#~ 
        #~ # rank available water
        #~ zoneMaxOrder = pcr.areaorder(cellAvlWater * -1.0, allocation_zones)
#~ 
        #~ # cell with maximum available water volume in each zone/segment (unit: m3)
        #~ cellWithMaxWater = pcr.ifthenelse(zoneMaxOrder == 1.0, cellAvlWater, 0.0)
        #~ # - using a safety factor so that not all of water will be used immediately
        #~ cellWithMaxWater = pcr.max(0.0, cellWithMaxWater * 0.75) 
#~ 
        #~ # abstraction (unit: m3)
        #~ abstractionFromCellWithMaxWater = pcr.min(zoneVolDemand, cellWithMaxWater)
        #~ 
        #~ # allocation water to meet water demand (unit: m3)
        #~ allocationFromCellWithMaxWater = getValDivZero(cellVolDemand, zoneVolDemand, smallNumber) * pcr.areamaximum(abstractionFromCellWithMaxWater, allocationZones)
        #~ 
        #~ # update abstraction and allocation
        #~ cellAbstraction += abstractionFromCellWithMaxWater
        #~ cellAllocation  += allocationFromCellWithMaxWater
        #~ 
        #~ # the remaining demand and available water
        #~ cellVolDemand = pcr.max(0.0, cellVolDemand - allocationFromCellWithMaxWater) 
        #~ cellAvlWater  = pcr.max(0.0, cellAvlWater  - abstractionFromCellWithMaxWater)
#~ 
        #~ if debug_water_balance:
        #~ 
            #~ waterBalanceCheck([pcr.cover(pcr.areatotal(cellAbstraction, allocationZones)/zone_area, 0.0)],\
                              #~ [pcr.cover(pcr.areatotal(cellAllocation , allocationZones)/zone_area, 0.0)],\
                              #~ [pcr.scalar(0.0)],\
                              #~ [pcr.scalar(0.0)],\
                              #~ 'abstraction - allocation per zone/segment (PS: Error here may be caused by rounding error.)' ,\
                               #~ True,\
    ...
    #~ cellVolDemand = pcr.min(1000., cellVolDemand)
    #~ cellAvlWater  = pcr.min(1000., cellAvlWater )
    
    cellVolDemand = pcr.rounddown(cellVolDemand)
    ...
    # total available water volume in each zone/segment (unit: m3)
    zoneAvlWater  = pcr.areatotal(cellAvlWater, allocationZones)
    ...
    # actual water abstraction volume in each cell (unit: m3)
    distributedCellAbstraction = pcr.rounddown(getValDivZero(cellAvlWater, zoneAvlWater, smallNumber)*1000.)/1000. * zoneAbstraction
    distributedCellAbstraction = pcr.min(distributedCellAbstraction, cellAvlWater)                                                                   

    # recalculate zoneAbstraction  
    zoneAbstraction = pcr.areatotal(distributedCellAbstraction, allocationZones) 

    # allocation water to meet water demand (unit: m3)
    distributedCellAllocation  = getValDivZero(cellVolDemand, zoneVolDemand, smallNumber) * zoneAbstraction 
    distributedCellAllocation  = pcr.min(distributedCellAllocation, cellVolDemand)
  
    # update abstraction and allocation
    cellAbstraction += distributedCellAbstraction
    cellAllocation  += distributedCellAllocation
