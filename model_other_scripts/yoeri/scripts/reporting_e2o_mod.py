...
class Reporting(object):
    ...
    def additional_post_processing(self):
        ...
        #===============================================================================================================
        # RvB edit 23/07/2018: replaced by a generic function to compute the values for irrigation provided that
        # irrigated land cover types are denoted by a name starting with "irr"
        # Some examples to report variables from certain land cover types:
        # - unit: m/day - values are average over the entire cell area
        self.precipitation_at_irrigation    = pcr.ifthen(self._model.routing.landmask, pcr.scalar(0.0))
        self.netLqWaterToSoil_at_irrigation = pcr.ifthen(self._model.routing.landmask, pcr.scalar(0.0))
        self.evaporation_from_irrigation    = pcr.ifthen(self._model.routing.landmask, pcr.scalar(0.0))
        self.transpiration_from_irrigation  = pcr.ifthen(self._model.routing.landmask, pcr.scalar(0.0))
        # Only update the maps if includeIrrigation is set to True
        if self._model.landSurface.includeIrrigation:
            # Iterate over the land cover classes and extract the corresponding classes
            irrigated_land_cover_types= []
            for land_cover_name in self._model.landSurface.landCoverObj.keys():
                if 'irr' in land_cover_name[:3]:
                    irrigated_land_cover_types.append(land_cover_name)
            irrigated_land_cover_types.sort()
            print irrigated_land_cover_types
            #-iterate over the land cover name in irrigated land cover types
            for land_cover_name in irrigated_land_cover_types:
                self.precipitation_at_irrigation    =   self.precipitation_at_irrigation \
                                                        + self._model.landSurface.landCoverObj[land_cover_name].fracVegCover \
                                                        * self._model.meteo.precipitation
                self.netLqWaterToSoil_at_irrigation =   self.netLqWaterToSoil_at_irrigation \
                                                        + self._model.landSurface.landCoverObj[land_cover_name].fracVegCover \
                                                        * self._model.landSurface.landCoverObj[land_cover_name].netLqWaterToSoil
                self.evaporation_from_irrigation    =   self.evaporation_from_irrigation \
                                                        + self._model.landSurface.landCoverObj[land_cover_name].fracVegCover \
                                                        * self._model.landSurface.landCoverObj[land_cover_name].actualET
                self.transpiration_from_irrigation  =   self.transpiration_from_irrigation \
                                                        + self._model.landSurface.landCoverObj[land_cover_name].fracVegCover \
                                                        * self._model.landSurface.landCoverObj[land_cover_name].actTranspiTotal
        # RvB end of edit 23/07/2018
        #===============================================================================================================
