...
class Meteo(object):
    ...
    def read_forcings(self,currTimeStep):
        # reading precipitation:	#deleted by joyce (files not stored per 10 years)
        #endYear = int(np.ceil(currTimeStep.year/10.)*10.)
        #startYear = endYear - 9
        #if self.preFileNC.find("hist") == -1:
        #  if endYear == 2010:
        #    startYear = 2006
        #    if self.preFileNC.lower().find("hadgem2") != -1:
        #      startYear = 2005
        #  if endYear > 2099:
        #    endYear = 2099
        #else:
        ...
        #  if endYear == 2010:
        #    endYear = 2005
        #  if self.preFileNC.lower().find("hadgem2") != -1 and startYear == 2001:
        #    endYear = 2004
        #self.preFile = "%s%d-%d.nc" %(self.preFileNC, startYear, endYear)
        ...
        # reading temperature
        #self.tmpFile = "%s%d-%d.nc" %(self.tmpFileNC, startYear, endYear)

        #EI (E2O) input Tair has an additional dimension: 
        print(str(self.tmpFileNC))
        if 'EI_1979_to_2014' in str(self.tmpFileNC):
            print('using alternate netcdf2PCRobj for file with more than 3 dims')
            self.temperature = vos.netcdf2PCRobjCloneMultiDimTair(\
