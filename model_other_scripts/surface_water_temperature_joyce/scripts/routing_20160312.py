...
class Routing(object):
    ...
    def __init__(self,iniItems,initialConditions,lddMap):
        ...
        self.max_water_height = 5000.000
	...
	self.water_height = pcr.min(self.max_water_height, channelStorageForRouting / (pcr.max(self.min_fracwat_for_water_height, self.dynamicFracWat) * self.cellArea))
        self.temp10 = self.water_height
        self.temp11 = self.channelStorage
        ...
        for i_loop in range(number_of_loops):
            ...
            self.temp10b = dischargeInitial
            self.temp10a = scalar(alpha)
            # at the lake/reservoir outlets, use the discharge of water bofy outflow
            waterBodyOutflowInM3PerSec = self.waterBodyOutflow / vos.secondsPerDay()
            ...
            channelStorageForRouting += storage_change_in_volume
            self.temp11a = self.subDischarge
            self.temp11b = channelStorageForRouting 
            #
            # route only non negative channelStorage (otherwise stay):
    ...
    def simple_update(self,landSurface,groundwater,currTimeStep,meteo):
        ...
        self.temp21 = self.runoff + self.nonIrrReturnFlow - landSurface.actSurfaceWaterAbstract -self.waterBodyEvaporation + \
                                 preStorage/self.cellArea - \
                                 self.channelStorage/self.cellArea
        if self.debugWaterBalance:\
        ...
        self.temp18 = waterBodyOutflow
        self.temp19 = self.channelStorage
        self.temp20 = self.channelStorage + waterBodyOutflow

        self.waterBodyOutflow = pcr.cover(waterBodyOutflow, 0.0)             # unit: m3/day
        #self.waterBodyOutflow = pcr.cover(pcr.min(-self.channelStorage,waterBodyOutflow), 0.0)             # unit: m3/day
        self.channelStorage += self.waterBodyOutflow
        self.temp25 = self.WaterBodies.temp
        ...
        if self.waterTemperature:
            ...
            #self.water_height = pcr.min(self.max_water_height, self.channelStorage / (pcr.max(self.min_fracwat_for_water_height, self.dynamicFracWat) * self.cellArea))
    ...
    def calculate_alpha_and_initial_discharge_for_kinematic_wave(self): 
        ...
        #
        self.temp16a = self.channelStorage
        self.temp16b = self.dynamicFracWat
        self.temp16c = self.iceThickness
        if self.waterTemperature and self.floodPlain: 
            ...
            self.temp16h = alpha
            self.temp17a = manningsWithIce*wetP
            self.temp17b = manningsWithIce*wetP**(2./3.)
            self.temp17c = self.gradient**-0.5
            self.temp17d = (manningsWithIce*wetP**(2./3.)*self.gradient**-0.5)
            self.temp17e = scalar(self.beta)
            #TODO alpha = (self.manningsN*wetP**(2./3.)*self.gradient**(-0.5))**self.beta            
        elif self.waterTemperature and self.floodPlain != True:
            print "Test"
            manIce= pcr.max(self.manningsN,\
              0.0493*pcr.max(0.01,self.channelStorage/(self.dynamicFracWat*self.cellArea))**\
              (-0.23)*self.iceThickness**0.57)
            ...
            alpha= (manningsWithIce*channel_wetted_perimeter**(2./3.)*self.gradient**-0.5)**self.beta
        else:
            channel_wetted_area      =   self.water_height * self.wMean                                  # unit: m2
            channel_wetted_perimeter = 2*self.water_height + self.wMean                                  # unit: m  
        #
    ...
    def kinematic_wave_update(self, landSurface,groundwater,currTimeStep,meteo): 
        ...
        for i_loop in range(number_of_loops):
            ...
            # update channelStorage (m3) after waterBodyOutflow (m3)
            #waterBodyOutflow = pcr.max(-channelStorageForRouting, waterBodyOutflow)
            ...
            waterBodyOutflowInM3PerSec = self.waterBodyOutflow / length_of_sub_time_step
    ...
    def energyLocal(self, meteo, landSurface, groundwater, timeSec = vos.secondsPerDay()):
        ...
        self.temp1 = self.totEW
        self.temp2 = self.temp_water_height
    ...
    def energyWaterBodyAverage(self):
        ...
        self.temp4 = self.temp_water_height
        self.temp5 = self.totEW
        self.temp6 = self.channelStorageNow
        ...
        self.temp13 = self.channelStorageNow
        self.temp14 = self.temp_water_height
        self.temp15 = self.return_water_body_storage_to_channel(pcr.max(self.channelStorageNow,0.0))/(self.dynamicFracWat * self.cellArea)
        self.waterTemp= pcr.ifthenelse(self.temp_water_height > self.critical_water_height,\
          self.totEW/self.temp_water_height/(self.specificHeatWater*self.densityWater),self.temperatureKelvin)
        self.waterTemp= pcr.ifthenelse(self.waterTemp < self.iceThresTemp+0.1,self.iceThresTemp+0.1,self.waterTemp)
        self.temp7 = self.waterTemp
        self.temp8 = self.temp_water_height
