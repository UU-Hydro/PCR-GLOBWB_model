...
def netcdf2PCRobjCloneMultiDimTair(ncFile,varName,dateInput,\
    ...
    print f.variables[varName].shape
    print int(idx)
...
def gdalwarpPCR_NIKO(input,output,cloneOut,tmpDir,isLddMap=False,isNominalMap=False):
    ...
    co = '/home/sutan101/anaconda2/bin/gdal_translate -ot Float64 '+str(input)+' '+str(tmpDir)+'tmp_inp.tif'
    if isLddMap == True: co = '/home/sutan101/anaconda2/bin/gdal_translate -ot Int32 '+str(input)+' '+str(tmpDir)+'tmp_inp.tif'
    if isNominalMap == True: co = '/home/sutan101/anaconda2/bin/gdal_translate -ot Int32 '+str(input)+' '+str(tmpDir)+'tmp_inp.tif'
    cOut,err = subprocess.Popen(co, stdout=subprocess.PIPE,stderr=open('/dev/null'),shell=True).communicate()
    # 
    ...
    co = '/home/sutan101/anaconda2/bin/gdalwarp '+te+tr+ \
         ' -srcnodata -3.4028234663852886e+38 -dstnodata mv '+ \
           str(tmpDir)+'tmp_inp.tif '+ \
           str(tmpDir)+'tmp_out.tif'
    cOut,err = subprocess.Popen(co, stdout=subprocess.PIPE,stderr=open('/dev/null'),shell=True).communicate()
    # 
    co = '/home/sutan101/anaconda2/bin/gdal_translate -of PCRaster '+ \
...
def waterBalanceCheck(fluxesIn,fluxesOut,preStorages,endStorages,processName,PrintOnlyErrors,dateStr,threshold=1e-5,landmask=None):
    ...
    a,b,c = getMinMaxMean(inMap + dsMap- outMap, True)
