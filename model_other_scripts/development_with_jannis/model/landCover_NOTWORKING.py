...
#~ 
    ...
    def calculateInfiltration(self):
        ...
        #
        #~ # - original line (before coupling to Jannis's hydrodynamic model)
        #~ self.directRunoff += pcr.max(0.0,\
                             #~ self.topWaterLayer - self.minTopWaterLayer)
        #
        # - modified in order to couple with inundation depth from routing (starting from 31 Oct 2017)
        flood_inundation_depth = 0.0     # TODO: get this from the routing.py module. 
        maximum_inundation     = pcr.max(flood_inundation_depth, self.minTopWaterLayer)
        self.directRunoff += pcr.max(0.0,\
                             self.topWaterLayer - maximum_inundation)
