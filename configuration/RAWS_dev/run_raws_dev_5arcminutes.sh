...
pixi run --manifest-path "$REPO_ROOT/pixi.toml" pcrglobwb-run-with-arguments \
    ...
    --output-dir "$DATA_DIR/output/5arcminutes/pcrglobwb" \
    --input-dir "$INPUT_DIR" \
    --clone-map "$CLONE_MAP" \
...
pixi run --manifest-path "$REPO_ROOT/pixi.toml" pcrglobwb-run-with-arguments \
    ...
    --output-dir "$DATA_DIR/output/5arcminutes/online_pcrglobwb_qualloc" \
    --input-dir "$INPUT_DIR" \
    --clone-map "$CLONE_MAP" \
    --qualloc-config "$REPO_ROOT/configuration/run_with_args/online_pcrglobwb_qualloc/qualloc_5arcminutes.cfg" \
    --log-level "$LOG_LEVEL" --file-level "$FILE_LEVEL"
